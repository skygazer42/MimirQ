...
def _run_smoke_test(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="mimirq-dr-") as tmp:
        ...
        # Never echo secrets (token/password) in the JSON report.
        ...
        if out_path.exists():
            try:
                report["report"] = json.loads(out_path.read_text(encoding="utf-8"))
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="DR restore verification: ready + smoke + index-audit (PII-safe).")

    p.add_argument("--base-url", default=os.getenv("NEXT_PUBLIC_API_URL", "http://localhost:8000"))
    p.add_argument("--tenant-id", default=os.getenv("NEXT_PUBLIC_TENANT_ID", "00000000-0000-0000-0000-000000000000"))

    p.add_argument("--auth-mode", default=None, help="Override auth mode (jwt|header).")
    p.add_argument("--user-id", default=os.getenv("NEXT_PUBLIC_USER_ID", ""), help="X-User-ID (AUTH_MODE=header).")
    p.add_argument("--token", default=os.getenv("MIMIRQ_DR_ADMIN_TOKEN", ""), help="Bearer token (AUTH_MODE=jwt).")
    ...
    if not bool(args.skip_index_audit):
        ...
        if (not dataset_id) and isinstance(smoke_report, dict):
            smoke_payload = smoke_report.get("report")
            if isinstance(smoke_payload, dict):
                dataset_id = str(smoke_payload.get("dataset_id") or "").strip()
        ...
        if not dataset_id:
            ...
            if not ok_http:
                ...
                if isinstance(body, dict):
                    ...
                    audit_ok = (vector_id_missing == 0) and (missing_in_backend == 0) and (orphan_count == 0)
                else:
                    audit_ok = False

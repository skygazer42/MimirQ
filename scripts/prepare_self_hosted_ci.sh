...
CACHE_ROOT="${RUNNER_TOOL_CACHE:-$RUNNER_TEMP}/mimirq"
VENV_DIR="${SELF_HOSTED_VENV_DIR:-$CACHE_ROOT/python-3.11}"
PIP_CACHE_DIR_VALUE="${PIP_CACHE_DIR:-$CACHE_ROOT/pip}"
...
if [ "$INCLUDE_TORCH_WHEEL_DIR" -eq 1 ]; then
  TORCH_WHEEL_DIR_VALUE="${TORCH_WHEEL_DIR:-$CACHE_ROOT/torch-wheels}"

...
- `changzhou_gov_dify_full_gate.py`: run the Changzhou government-service Dify/MimirQ golden gate (preflight, generated answers, direct eval, workflow trace)
  ...
  - Quick diagnosis: `make changzhou-dify-readiness-status` prints the latest summary's pass/fail state, freshness, boundary verdict, direct MimirQ base URL match state, non-blocking warning counters, root cause, next action, skipped stages, and artifact paths.

...
- `run_retention_jobs.py`: bounded tenant-scoped retention for database, knowledge assets, and semantic cache storage
  - Semantic cache dry-run: `python scripts/run_retention_jobs.py --semantic-cache --tenant-id <uuid> --dry-run --max-scan 1000 --max-delete 100`
  - Use `--execute` only after reviewing the JSON plan; Milvus maintenance failures produce a non-zero exit code.
...
- `dify_console_login.py`: refresh the Dify console Playwright `storage_state` used by workflow trace diagnostics
  ...
  - Avoid putting Dify console passwords in repository files or shell history; the script writes only `console_token` localStorage state to `/tmp/dify_console_storage_state.json` by default.
  - The Dify console web UI is served under the `/brainai` base path, for example `https://dify.example.com:3000/brainai/apps`. Keep `DIFY_CONSOLE_ORIGIN=https://dify.example.com:3000` because browser storage state is keyed by origin, not by path.
- `changzhou_gov_dify_external_knowledge_probe.py`: compare Dify external hit-testing with direct MimirQ retrieval for the same Changzhou golden cases
  - MimirQ-only preflight: `make changzhou-dify-mimirq-direct-gate` runs the same golden retrieval cases directly against MimirQ using `DIFY_EXTERNAL_KNOWLEDGE_API_KEY(S)` from `.env`, without Dify console auth. The default command enables the `changzhou-retrieval` quality profile, so it gates hit rates, evidence quality (`answer_grounding_rate`, key-point recall, effective context rate, and noise rate), and expected-metadata coverage/match rates for cases that declare `expected.metadata`.
  - Existing-report gate: `python scripts/changzhou_gov_golden_eval.py --report /tmp/report.json --quality-profile changzhou-retrieval` rechecks a saved report without calling MimirQ or requiring a token.
  - KG-on/KG-off comparison gate: `make changzhou-dify-kg-on-off-gate` first writes KG-off and KG-on MimirQ-only reports with `--kg-mode off/on`, then compares them. The saved-report-only form remains available as `CHANGZHOU_DIFY_KG_BASELINE_REPORT=/tmp/kg-off.json CHANGZHOU_DIFY_KG_CANDIDATE_REPORT=/tmp/kg-on.json CHANGZHOU_DIFY_KG_COMPARE_OUT=/tmp/changzhou_gov_dify_kg_compare.json make changzhou-dify-kg-compare-gate`.
  - `--kg-mode` is a MimirQ direct-gate extension: `default` inherits service settings; `off/on` sends request-level KG overrides to the Dify adapter. Standard Dify requests can omit these fields.
  - The compare gate verifies the KG-on candidate still passes `changzhou-retrieval`, does not regress baseline hit/grounding/effective-context/metadata metrics, and keeps `kg_noise_rate <= 0.1`.
  - Local route preflight: `make changzhou-dify-knowledge-map-check` validates `DIFY_EXTERNAL_KNOWLEDGE_MAP_JSON` before remote Dify calls, including declared `plugin_refs` and their `retrieval_policy` availability.
  - Its summary exposes `plugin_refs_checked`, `plugin_refs_invalid`, and `plugin_refs_missing_retrieval_policy` for CI/readiness triage.
...
- `changzhou_gov_dify_full_gate.py`: run the Changzhou government-service Dify/MimirQ golden gate (preflight, generated answers, direct eval, workflow trace)
  ...
  - End-to-end readiness: `make changzhou-dify-readiness-gate` runs the local knowledge-map preflight, Dify console token check, strict external probe, then the full generated-answer/direct-eval/trace gate. It inherits the `changzhou-retrieval` profile for direct retrieval quality and keeps generated-answer grounding/key-point recall thresholds defaulting to `0.9` to avoid failing on harmless wording variance, then writes `/tmp/changzhou_gov_dify_readiness_summary.json`.
  - If `CHANGZHOU_DIFY_KG_COMPARE_OUT` points to an existing compare artifact, `make changzhou-dify-readiness-summary` includes it as the optional `kg_compare` stage and readiness status/evidence prints `kg_noise_rate` plus candidate-gate status.
  - Terminal-safe readiness: `make changzhou-dify-readiness-gate-quiet` runs the same gate but redirects raw output to `/tmp/changzhou_gov_dify_readiness_gate.log`, then prints only the compact readiness status.
  ...
  - Reads `DIFY_EXTERNAL_KNOWLEDGE_API_KEY` / `DIFY_EXTERNAL_KNOWLEDGE_API_KEYS` from the environment or repo `.env`; Dify App key and console storage state default to `/tmp/dify_remote_app_api_key.json` and `/tmp/dify_console_storage_state.json`.

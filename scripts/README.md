...
- `changzhou_gov_dify_full_gate.py`: run the Changzhou government-service Dify/MimirQ golden gate (preflight, generated answers, direct eval, workflow trace)
  ...
  - If an upstream stage fails, the readiness summary reports only the root cause in `failed_stages` and `root_cause_stage/root_cause_reason`; downstream stages are marked `status=skipped` with `blocked_by=<stage>`, so expired Dify console auth is not hidden behind skipped probe/full-gate stages.

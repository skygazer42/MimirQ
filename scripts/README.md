...
- `changzhou_gov_dify_full_gate.py`: run the Changzhou government-service Dify/MimirQ golden gate (preflight, generated answers, direct eval, workflow trace)
  ...
  - If Dify console auth fails or expires soon, the readiness summary now reports `console_auth` as its own failed stage instead of hiding the root cause behind skipped probe/full-gate stages.

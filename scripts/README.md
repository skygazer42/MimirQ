...
- `dify_console_login.py`: refresh the Dify console Playwright `storage_state` used by workflow trace diagnostics
  ...
  - The example-org Dify console web UI is served under the `/brainai` base path, for example `https://dify.example.com:3000/brainai/apps`. Keep `DIFY_CONSOLE_ORIGIN=https://dify.example.com:3000` because browser storage state is keyed by origin, not by path.

...
def convert_evidence_pack_to_regression_bundle(
    ...
    *,
    ...
    ds = str(dataset_id or pack.get("dataset_id") or "").strip()
    if not ds:
        raise ValueError("dataset_id is required")

    q = str(question or pack.get("query") or "").strip()
    if not q:
        raise ValueError("query is required")

    # Prefer explicit reference_sources exported from the UI; fall back to building them from
    # (selected_chunk_ids + citations).
    refs = list(_iter_dicts(pack.get("reference_sources")))
    if not refs:
        refs = _build_reference_sources_from_citations(pack)

...
@pytest.mark.asyncio
async def test_agentic_runner_emits_critic_metrics_after_answer(
    ...
    agentic_plan_step_cls = rag_agent_mod.AgenticPlanStep
    agentic_rag_runner_cls = rag_agent_mod.AgenticRAGRunner
    ...
    monkeypatch.setattr(settings, "RAG_CRITIC_ENABLED", True, raising=False)

    engine = RAGEngine()
    runner = agentic_rag_runner_cls(engine=engine)

    async def _fake_plan(*_args, **_kwargs):  # noqa: ANN003
        return [agentic_plan_step_cls(query="focused retrieval query", rationale="Need one grounded answer.")]
    ...
    agen = runner.stream(
        ...
        request_id="agentic-critic-test",
    )
    ...
    assert done_metrics.get("agentic_critic_used") is True
    assert done_metrics.get("agentic_critic_verdict") == "accept"
    assert done_metrics.get("agentic_critic_citation_missing") is False

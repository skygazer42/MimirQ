...
def test_embedding_blue_green_migration_script_help_runs_directly() -> None:
    script = Path(__file__).resolve().parents[1] / "scripts" / "embedding_blue_green_migrate.py"
    ...
    assert "embedding blue-green migration helpers" in result.stdout

...
def test_delete_chunk_indexes_strict_deletes_plausible_scoped_collection_for_ambiguous_metadata(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from app.services import indexer
    ...
    class _Adapter:
        ...
        def delete_by_document_id(self, doc_id, tenant_id=None) -> None:  # noqa: ANN001
            ...
            deleted.append((self.collection_name, doc_id))
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            delete_by_document_id=lambda doc_id, tenant_id=None: deleted.append(("documents", doc_id))
        ),
    )
    monkeypatch.setattr(
        type(indexer.hybrid_retriever),
    ...
    service._touch_chunk_retrieval_scope = lambda **_kwargs: None  # type: ignore[method-assign]

    service.delete_chunk_indexes(tenant_id=tenant_id, document_id=document_id, strict=True)

    assert deleted == [
        ("documents_emb_space_default", document_id),
...
def test_delete_dataset_scoped_chunk_vectors_deletes_ambiguous_default_space_from_plausible_scoped_collection(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from app.services import indexer
    from app.services.dataset_embedding_config import DatasetEmbeddingRuntimeConfig
    ...
    assert deleted == ["documents_emb_space_default"]

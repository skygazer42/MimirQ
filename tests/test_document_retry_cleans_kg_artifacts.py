...
@pytest.mark.asyncio
async def test_retry_document_processing_cleans_kg_artifacts(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    chunk_delete_query = _FakeQuery(delete_count=5)
    rel_delete_query = _FakeQuery(delete_count=2)
    db = _FakeDB([doc_query, chunk_delete_query, rel_delete_query])

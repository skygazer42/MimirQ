...
def test_parsing_openapi_schema_exposes_typed_elements_pages_field() -> None:
    ...
    source_backend = properties.get("source_backend") or {}
    source_element_id = properties.get("source_element_id") or {}
    ...
    source_backend_any_of = source_backend.get("anyOf") if isinstance(source_backend, dict) else None
    source_backend_string = next(
        (item for item in (source_backend_any_of or []) if isinstance(item, dict) and item.get("type") == "string"),
        {},
    )
    source_element_id_any_of = source_element_id.get("anyOf") if isinstance(source_element_id, dict) else None
    source_element_id_string = next(
        (item for item in (source_element_id_any_of or []) if isinstance(item, dict) and item.get("type") == "string"),
        {},
    )
    ...
    assert source_backend_string.get("type") == "string"
    assert source_element_id_string.get("type") == "string"

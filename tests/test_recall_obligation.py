...
def test_build_recall_obligation_ledger_tracks_source_and_anchor_coverage() -> None:
    ...
    assert int((ledger["anchors"] or {}).get("considered_citations") or 0) >= 0
    assert int((ledger["anchors"] or {}).get("skipped_citations") or 0) >= 0
    assert isinstance((ledger["anchors"] or {}).get("skipped_by_role") or {}, dict)
...
def test_build_must_recall_proof_is_versioned_and_self_contained() -> None:
    ...
    assert proof.get("matched_by_required_source_key") == {"sales": "sales"}
    assert list(proof.get("matched_source_keys") or []) == ["sales"]
    assert int(proof.get("anchor_considered_citations") or 0) >= 0
    assert int(proof.get("anchor_skipped_citations") or 0) >= 0

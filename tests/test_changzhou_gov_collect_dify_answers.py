...
def test_request_json_bypasses_proxy_for_private_dify_url(monkeypatch) -> None:
    ...
    result = mod._request_json(
        url="http://192.0.2.6:5001/v1/chat-messages",
        payload={"query": "q"},
        api_key="secret-token",
        timeout=12.0,
    )
    ...
    assert calls["url"] == "http://192.0.2.6:5001/v1/chat-messages"
    assert calls["timeout"] == 12.0
    assert calls["handlers"]


def test_request_json_bypasses_proxy_for_example-org_dify_host(monkeypatch) -> None:
    mod = _load_module()
    calls: dict[str, object] = {}
    ...
    result = mod._request_json(
        url="https://dify.example.com:5001/v1/chat-messages",
        payload={"query": "q"},
        api_key="secret-token",
        timeout=12.0,
    )
    ...
    assert result == {"answer": "ok"}
    assert calls["url"] == "https://dify.example.com:5001/v1/chat-messages"
    assert calls["timeout"] == 12.0
    assert calls["handlers"]

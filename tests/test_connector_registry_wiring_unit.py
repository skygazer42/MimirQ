...
async def test_check_db_connectivity_uses_connector_registry(monkeypatch: pytest.MonkeyPatch) -> None:
    import app.api.v1.connectors_validation as connectors_validation_module
    ...
    monkeypatch.setattr(
        connectors_validation_module.connector_class_registry,
        "get",
    ...
    check, warnings = await connectors_validation_module._check_db_connectivity_best_effort(
...
async def test_check_db_connectivity_returns_empty_when_not_registered(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(connectors_validation_module.connector_class_registry, "get", _raise_not_found, raising=True)

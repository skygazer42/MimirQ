from app.connectors.db.introspection import mysql_list_columns_sql, mysql_list_tables_sql, sqlserver_list_tables_sql
...
def test_mysql_columns_sql_is_select_only():
    sql = mysql_list_columns_sql(database="demo")
    assert "select" in sql.lower()
    assert ";" not in sql

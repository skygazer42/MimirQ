...
def _patch_common_retrieval_settings(monkeypatch) -> None:  # noqa: ANN001
...
def test_grounded_strict_profile_enforces_span_required_gate(monkeypatch) -> None:  # noqa: ANN001
    import app.rag.retrieval.orchestrator as orch_mod

    _patch_common_retrieval_settings(monkeypatch)
...
def test_grounded_strict_profile_triggers_refusal_semantics_when_no_evidence(monkeypatch) -> None:  # noqa: ANN001
    import app.rag.retrieval.orchestrator as orch_mod
    ...
    metrics = out.get("metrics") or {}
    assert metrics.get("retrieval_contract_mode") == "evidence_strict"
    assert metrics.get("abstain_enabled") is True
    assert metrics.get("abstain_triggered") is True
    assert metrics.get("abstain_reason") == "citations_lt_min"
    assert isinstance(metrics.get("abstain_followup"), dict)

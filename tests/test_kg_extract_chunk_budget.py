...
@pytest.mark.asyncio
async def test_kg_extract_uniform_chunk_budget_limits_long_documents(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def _fake_extract(self, sections, batch_index, **_kwargs):  # noqa: ANN001
        ...
        return [
            {
                "title": f"t{sections[0].chunk_index}",
                "summary": "s",
                "content": "c" * 50,
    ...
    assert len(events) == 3
    assert processed_chunk_indices == [0, 4, 9]

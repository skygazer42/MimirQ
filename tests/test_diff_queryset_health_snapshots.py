...
def test_diff_queryset_health_snapshots_script_writes_json(tmp_path: Path) -> None:
        ...
        json.dumps(
            {
                ...
                "risk": {
                    ...
                    "weak_hit_rate": 0.2,
                    "hard_cases": [{"id": "q-1"}],
                    "parse_risk_tail": [{"document_id": "doc-a", "score": 0.2}],
                },
                "degradation_flags": [],
            },
            ensure_ascii=False,
        ...
        json.dumps(
            {
                ...
                "risk": {
                    ...
                    "weak_hit_rate": 0.3,
                    "hard_cases": [{"id": "q-2"}],
                    "parse_risk_tail": [{"document_id": "doc-b", "score": 0.1}],
                },
    ...
    tail = payload.get("parse_risk_tail_drift") or {}
    assert tail.get("added_document_ids") == ["doc-b"]
    assert tail.get("removed_document_ids") == ["doc-a"]
...
def test_diff_queryset_health_snapshots_script_writes_markdown(tmp_path: Path) -> None:
    ...
    assert "q-2" in text
    assert "Parse Risk Tail Drift" in text

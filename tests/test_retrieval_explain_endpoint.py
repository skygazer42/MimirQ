...
async def test_retrieval_explain_endpoint_returns_deterministic_schema_payload(
    ...
    def _run_retrieval(_state):  # noqa: ANN001
        return {
            ...
            "query_debug": {
                "channels": {"vector": {"count": 5}, "keyword": {"count": 2}},
                "hierarchy_recall": {"enabled": True, "family_collapse": True},
            },
            "metrics": {
    ...
    assert res.hierarchy_recall.get("enabled") is True
    assert res.hierarchy_recall.get("family_collapse") is True
...
@pytest.mark.asyncio
async def test_retrieval_explain_endpoint_passes_hierarchy_recall_fields_into_state(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from app.core.config import settings
    ...
    def _run_retrieval(_state):  # noqa: ANN001
        return {"query_for_retrieval": "q_norm", "citations": [], "query_debug": {}, "metrics": {}}

    monkeypatch.setattr(api_mod, "build_rag_state", _build_rag_state, raising=True)
    monkeypatch.setattr(api_mod, "run_retrieval", _run_retrieval, raising=True)

    body = api_mod.RetrievalExplainRequest(
        query="What is recall?",
        rag_config={"retrieval_profile": "hierarchy_recall20"},
    )
    _ = await api_mod.explain_retrieval(
        body=body,
        tenant_id=uuid.uuid4(),
        account_id="u",
        db=None,
    ...
    assert captured.get("hierarchy_family_aggregation") == "combined"
    assert captured.get("hierarchy_tree_dedup") is True
    assert captured.get("hierarchy_parent_depth") == 0
    assert captured.get("hierarchy_sibling_window") == 0

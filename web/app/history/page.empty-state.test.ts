...
describe('history page empty state', () => {
  it('offers a clear next action via the shared messages catalog', () => {
    ...
    expect(src).toContain('bg-[linear-gradient(135deg,hsl(var(--info)/0.08),transparent_42%),radial-gradient(circle_at_50%_105%,hsl(var(--info)/0.10),transparent_45%)]')
    expect(src).not.toContain('rgba(14,165,233')

...
describe('history page message bubbles', () => {
  it('renders history messages through the minimal chat variant while keeping explicit bubble surfaces for both roles', () => {
    ...
    expect(messageSrc).toContain("max-w-none break-words leading-relaxed text-black [&>*]:text-black")
    expect(messageSrc).toContain('whitespace-pre-wrap font-normal text-black')

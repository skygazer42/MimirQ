...
export function DifyIntegrationSection({
  ...
  return (
    <section className="space-y-3">
      <div className="rounded-[16px] border border-blue-100/80 bg-gradient-to-br from-blue-50/55 via-white to-cyan-50/35 p-3.5">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <span className="flex size-8 items-center justify-center rounded-[12px] border border-blue-100 bg-white text-blue-600 shadow-sm">
          ...
          <div className="flex items-center gap-2 rounded-full border border-blue-100 bg-white/80 px-2.5 py-1.5 text-[11px] text-slate-600">
        ...
        <div className="mt-3 grid grid-cols-1 gap-2.5 lg:grid-cols-[minmax(0,1fr)_220px]">
          <div className="rounded-[13px] border border-slate-200/75 bg-slate-950 px-3 py-2.5 text-white shadow-sm">
            <div className="flex items-center justify-between gap-2">
              <div className="min-w-0">
                <div className="text-[10px] font-semibold uppercase tracking-[0.14em] text-blue-200/90">
                  Retrieval Endpoint
                </div>
                <div className="mt-1 truncate font-mono text-[12px] text-slate-50">
              ...
              <Button
                type="button"
                variant="outline"
                className="h-8 shrink-0 rounded-[11px] border-white/15 bg-white/8 px-2.5 text-[11px] text-white hover:bg-white/14 hover:text-white"
          ...
          <div className="rounded-[13px] border border-slate-200/75 bg-white/85 px-3 py-2.5">
            ...
            <Input
              ...
              className="mt-1.5 h-8 rounded-[10px] border-slate-200 bg-white text-[12px]"
      ...
      <div className="grid grid-cols-1 gap-3 xl:grid-cols-[minmax(0,1.45fr)_minmax(320px,0.9fr)]">
        <div className="rounded-[16px] border border-slate-200/75 bg-white/82 p-3.5">
          ...
          <div className="mt-3 grid max-h-[260px] grid-cols-1 gap-2 overflow-auto pr-1 md:grid-cols-2">
            {datasetsLoading ? (
              <div className="col-span-full flex items-center gap-2 rounded-[12px] border border-slate-200 bg-slate-50 px-3 py-2 text-[12px] text-slate-500">
                <RefreshCw className="size-3.5 animate-spin" />
                正在加载数据集
              </div>
            ) : null}
            {datasetsError ? (
              <div className="col-span-full rounded-[12px] border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] text-amber-700">
                {datasetsError}
              </div>
            ) : null}
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    checked
                      ? 'border-blue-200 bg-blue-50/75 text-blue-700'
                      : 'border-slate-200 bg-white text-slate-700 hover:border-blue-100 hover:bg-blue-50/35'
                  )}
                >
                  <span
                    className={cn(
                  ...
                  <span className="min-w-0">
                    ...
                    <span className="block font-mono text-[10.5px] text-slate-500">
                      {shortId(dataset.id)}
                    </span>
                  </span>
                </button>
          ...
          <div className="mt-3 grid grid-cols-1 gap-2.5 md:grid-cols-[minmax(0,1fr)_auto]">
            <Input
              value={knowledgeId}
              className="h-9 rounded-[11px] border-slate-200 bg-white text-[12px]"
              placeholder="knowledge_id"
              onChange={(event) => setKnowledgeId(event.target.value)}
            />
            <Button
              type="button"
              className="h-9 rounded-[11px] bg-blue-600 px-3 text-[12px] font-medium text-white hover:bg-blue-700"
        ...
        <div className="space-y-3 rounded-[16px] border border-slate-200/75 bg-white/82 p-3.5">
          <div className="flex items-center justify-between gap-2">
            <div>
          ...
          {knowledgeMapError ? (
            <div className="rounded-[12px] border border-amber-200 bg-amber-50 px-3 py-2 text-[12px] text-amber-700">
              {knowledgeMapError}
            </div>
          ) : null}
          ...
          <div className="space-y-2">
            ...
            {Object.entries(knowledgeMap).map(([bindingId, datasetIds]) => (
              <div
                key={bindingId}
                className="rounded-[13px] border border-slate-200 bg-slate-50/65 px-3 py-2.5"
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="min-w-0">
                    <div className="font-mono text-[12px] font-semibold text-slate-900">{bindingId}</div>
                    <div className="mt-1 flex flex-wrap gap-1.5">
                      {datasetIds.map((datasetId) => (
                        <span
                          key={datasetId}
                          className="rounded-full border border-blue-100 bg-white px-2 py-0.5 text-[10.5px] text-slate-600"
                        >
                  ...
                  <Button
                    type="button"
                    variant="ghost"
                    className="h-7 w-7 shrink-0 rounded-[9px] text-slate-400 hover:bg-rose-50 hover:text-rose-600"
          ...
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2 xl:grid-cols-1">
            <label className="block text-[11px] leading-4 text-slate-600">
              ...
              <Input
                ...
                className="mt-1 h-8 rounded-[10px] border-slate-200 bg-white text-[12px]"

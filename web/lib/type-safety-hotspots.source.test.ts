...
describe('type safety hotspots', () => {
  ...
  it('keeps static skeleton maps off index keys in the remaining hotspot views', () => {
    ...
    const bareIndexKey = 'key={' + 'index}'

    expect(graphCanvas).not.toContain(bareIndexKey)
    expect(feedbackPage).not.toContain(bareIndexKey)
    expect(ingestionPage).not.toContain(bareIndexKey)
    expect(similarityWorkbench).not.toContain(bareIndexKey)
  })

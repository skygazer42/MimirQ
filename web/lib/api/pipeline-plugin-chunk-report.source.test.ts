...
describe('pipeline plugin chunk report API client source', () => {
  it('posts registered plugin sample requests to the chunk-report endpoint', () => {
    ...
    expect(src).toContain('normalizePipelinePluginChunkReportReadiness')
    expect(src).toContain('readiness: normalizePipelinePluginChunkReportReadiness(raw.readiness)')

...
describe('parsing api source', () => {
  ...
  it('defines extraction request and response contracts for parsing fields', () => {
    ...
    expect(src).toContain('source_visual_kind?: string | null')
    expect(src).toContain('source_visual_kind: spec?.source_visual_kind ?? undefined')
    expect(src).toContain('source_visual_kind: z.string().nullable().optional()')

    ...
    <Card className={cn(loading ? 'opacity-60' : '')}>
      ...
      <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center justify-between gap-2 md:col-span-2">
              <div className="text-sm">
                ...
                <div className="text-muted-foreground text-xs">禁用后 extractor 会将其归一化为 unknown（或丢弃）。</div>
              </div>
        ...
        <div className="rounded-xl border border-border overflow-hidden">
          <div className="grid grid-cols-[1fr_auto] gap-2 px-4 py-2 bg-muted text-xs text-muted-foreground">
            <div>predicate</div>
            <div>enabled</div>
          </div>
          <div className="divide-y divide-border">
            {rows.length === 0 ? (
              ...
              rows.map((r) => (
                <div key={r.id} className="grid grid-cols-[1fr_auto] gap-2 px-4 py-3 items-center">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="font-mono text-xs truncate" title={r.predicate}>
                        {r.predicate}
                      </span>
                      {r.display_name ? (
                        <span className="text-[11px] text-muted-foreground truncate" title={r.display_name}>
                          {r.display_name}
                        </span>
                      ) : null}
                    </div>
                    {r.description ? (
                      <div className="text-[11px] text-muted-foreground truncate" title={r.description}>
                        {r.description}
                      </div>
                    ) : null}
                    <div className="mt-1 flex items-center gap-2">
                      <Button type="button" size="sm" variant="outline" onClick={() => startEdit(r)} className="h-7 px-2 text-[11px] gap-1">
                      ...
                      <Button
                        ...
                        className="h-7 px-2 text-[11px] gap-1 hover:bg-destructive/10 hover:text-destructive"
                      >

...
describe('chat area autorun source', () => {
  ...
  it('submits document-selection and follow-up actions to the backend instead of only prefilling', () => {
    ...
    const pageClientSrc = fs.readFileSync(path.resolve(__dirname, 'chat-page-client.tsx'), 'utf8')
    ...
    expect(src).not.toContain('if (isLoading || messages.length > 0) return')
    expect(messageSrc).toContain("globalEventBus.emit('chat:submit', prompt)")
    expect(floatingMenuSrc).toContain('const handled = globalEventBus.emit("chat:submit", prompt)')
    expect(floatingMenuSrc).toContain('if (handled === 0) {')
    expect(floatingMenuSrc).toContain('params.set("doc", documentId)')
    expect(floatingMenuSrc).toContain('params.set("autorun", "1")')
    expect(pageClientSrc).toContain("params.delete('prompt')")
    expect(pageClientSrc).toContain("params.delete('autorun')")
  })

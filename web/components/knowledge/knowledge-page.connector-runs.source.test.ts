...
describe('KnowledgePage connector run placement', () => {
  it('keeps connector run monitoring inline in the documents workbench and leaves the settings tab focused on configuration only', () => {
    ...
    expect(settingsSection).toContain('<KnowledgeSettingsPanel')
    expect(settingsSection).toContain('selectedDatasetId={selectedDatasetId}')
    expect(settingsSection).toContain('selectedDataset={selectedDataset}')
    expect(settingsSection).toContain('datasets={datasets}')
    expect(settingsSection).toContain('datasetsLoading={datasetsLoading}')
    expect(settingsSection).toContain('datasetAllValue={DATASET_ALL}')
    expect(settingsSection).toContain('onDatasetScopeChange={handleDatasetScopeChange}')

    ...
    >
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(220px,1fr)_minmax(260px,1.6fr)_auto] lg:items-end">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            variant="outline"
            className="h-9 gap-2 rounded-lg border-slate-200 bg-card px-3 text-[12px] font-semibold text-slate-600"
            disabled
          >
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <Pill
          ...
          label="Schemas"
          onClick={() =>
            runAction('schemas', 'SCIM Schemas', () =>
              scimApi.listSchemas(base)
            )
          }
        />
        <Pill
          busy={busy === 'resource-types'}
          disabled={baseDisabled}
          label="ResourceTypes"
          onClick={() =>
            runAction('resource-types', 'SCIM ResourceTypes', () =>
              scimApi.listResourceTypes(base)
            )
          }
        />
        <Pill
          ...
          label="Group 映射"
          onClick={() =>
            runAction('groups', 'SCIM Group 列表', () =>
              scimApi.listGroups({ ...base, count: 50 })
            )
          }
        />
        <Pill
          ...
          label="User 映射"
          onClick={() =>
            runAction('users', 'SCIM User 列表', () =>
              scimApi.listUsers({ ...base, count: 50 })
            )
          }
        />
      </div>
      ...
      <details className="mt-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3">
        ...
        <p className="mt-1 text-[12px] leading-5 text-slate-500">
          仅在需要定位单个 User/Group 或执行创建、替换、Patch、删除时填写。
        </p>
        <div className="mt-3 grid gap-2 md:grid-cols-2">
          <Field label="Group">
            <Input
              ...
              className="h-8 rounded-lg border-slate-200 bg-card font-mono text-xs shadow-none"
            />
          </Field>
          <Field label="User">
            <Input
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <ActionButton
            busy={busy === 'group'}
            disabled={baseDisabled || !groupId.trim()}
            label="Group 详情"
            onClick={() =>
              runAction('group', 'SCIM Group 详情', () =>
                scimApi.getGroup({ ...base, groupId: groupId.trim() })
              )
            }
          />
          <ActionButton
            busy={busy === 'create-group'}
            disabled={baseDisabled}
            label="创建 Group"
            onClick={() =>
              runAction('create-group', '创建 SCIM Group', () =>
                scimApi.createGroup({
                  ...base,
                  payload: parseJson(groupPayload),
                })
              )
            }
          />
          <ActionButton
            busy={busy === 'update-group'}
            disabled={baseDisabled || !groupId.trim()}
            label="替换 Group"
            onClick={() =>
              runAction('update-group', '替换 SCIM Group', () =>
                scimApi.updateGroup({
                  ...base,
          ...
          />
          <ActionButton
            busy={busy === 'patch-group'}
            disabled={baseDisabled || !groupId.trim()}
            label="Patch Group"
            onClick={() =>
              runAction('patch-group', 'Patch SCIM Group', () =>
                scimApi.patchGroup({
                  ...base,
                  groupId: groupId.trim(),
                  payload: parseJson(groupPayload),
          ...
          <ConfirmDialog
            ...
            onConfirm={() =>
              runAction('delete-group', '删除 SCIM Group', () =>
                scimApi.deleteGroup({ ...base, groupId: groupId.trim() })
              )
            }
          >
            <Button
              variant="outline"
              className="h-8 gap-1.5 rounded-lg border-slate-200 bg-card px-3 text-xs font-semibold"
              disabled={baseDisabled || !groupId.trim()}
            >
              {busy === 'delete-group' ? (
          ...
          <ActionButton
            busy={busy === 'user'}
            disabled={baseDisabled || !userId.trim()}
            label="User 详情"
            onClick={() =>
              runAction('user', 'SCIM User 详情', () =>
                scimApi.getUser({ ...base, userId: userId.trim() })
              )
            }
          />
          <ActionButton
            ...
            label="创建 User"
            onClick={() =>
              runAction('create-user', '创建 SCIM User', () =>
                scimApi.createUser({ ...base, payload: parseJson(userPayload) })
              )
            }
          />
          <ActionButton
            busy={busy === 'patch-user'}
            disabled={baseDisabled || !userId.trim()}
            label="Patch User"
            onClick={() =>
              runAction('patch-user', 'Patch SCIM User', () =>
                scimApi.patchUser({
                  ...base,
                  userId: userId.trim(),
                  payload: parseJson(userPayload),
                })
              )
            }
          />
        ...
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
          <Field label="Group 请求体（JSON）">
            <Textarea
              value={groupPayload}
              onChange={(event) => setGroupPayload(event.target.value)}
              className="min-h-[128px] rounded-lg border-slate-200 bg-card font-mono text-xs shadow-none"
            />
          </Field>
      ...
      <OperationResultPanel
        ...
        title="SCIM 结果"
        result={result}
        emptyMessage="输入租户和 token 后执行操作；这里展示摘要，原始响应默认收起。"
      />
    </Panel>
  )
}
...
function Pill({
  ...
  return (
    <Button
      variant="outline"
      className="h-7 gap-1.5 rounded-full border-slate-200 bg-slate-50 px-3 text-[11px] font-semibold text-slate-600 shadow-none hover:bg-card"
      disabled={disabled}
      onClick={() => detachPromise(onClick())}
    >
      {busy ? (

  ...
  return (
    ...
    >
      ...
      <details className="mt-3 rounded-xl border border-slate-200/80 bg-slate-50/60 p-3">
        ...
        <p className="mt-1 text-[12px] leading-5 text-slate-500">
          仅在联调具体 IdP、ACS 或手动交换 SAML Response 时填写。
        </p>
        <div className="mt-3 grid gap-2 md:grid-cols-3">
          ...
          <Field label="Relay State">
            <Input
              value={relayState}
              onChange={(event) => setRelayState(event.target.value)}
              className="h-8 rounded-lg border-slate-200 bg-card font-mono text-xs shadow-none"
            />
          </Field>
          <Field label="ACS URL">
            <Input
              value={acsUrl}
              onChange={(event) => setAcsUrl(event.target.value)}
              className="h-8 rounded-lg border-slate-200 bg-card font-mono text-xs shadow-none"
            />
          </Field>
        </div>
        ...
        <Field label="SAML Response">
          <Textarea
            value={samlResponse}
            onChange={(event) => setSamlResponse(event.target.value)}
            className="mt-3 min-h-[104px] rounded-lg border-slate-200 bg-card font-mono text-xs shadow-none"
          />
        </Field>
        <div className="mt-3 flex flex-wrap gap-2">
          <ConfirmDialog
            ...
            onConfirm={() =>
              runAction('exchange', 'SAML Exchange', async () => {
          ...
          >
            <Button
              ...
              disabled={Boolean(busy) || !samlResponse.trim()}
            >
              {busy === 'exchange' ? (
      ...
      <OperationResultPanel
        ...
        title="SAML 操作结果"
        result={result}
        emptyMessage="获取 metadata 或执行 exchange 后，这里展示脱敏摘要；原始响应默认收起。"
      />
    </Panel>
  )
}

...
const SETTINGS_SWITCH_TRACK =
  'relative inline-flex h-[1.875rem] w-20 shrink-0 items-center overflow-hidden rounded-full border shadow-none transition-[background-color,border-color,transform] duration-150'

const SETTINGS_SWITCH_STATE =
  "[--switch-translate-checked:2.5rem] [--switch-translate-unchecked:0.25rem] before:pointer-events-none before:absolute before:left-2.5 before:top-1/2 before:z-20 before:-translate-y-1/2 before:text-[10px] before:font-semibold before:leading-none before:tracking-[0.04em] before:content-['停用'] before:transition-colors before:duration-150 after:pointer-events-none after:absolute after:right-2.5 after:top-1/2 after:z-20 after:-translate-y-1/2 after:text-[10px] after:font-semibold after:leading-none after:tracking-[0.04em] after:content-['启用'] after:transition-colors after:duration-150 data-[switch-state=checked]:before:text-muted-foreground/65 data-[switch-state=checked]:after:text-info-foreground data-[switch-state=unchecked]:before:text-foreground/80 data-[switch-state=unchecked]:after:text-muted-foreground/65"

const SETTINGS_SWITCH_TONE =
  'bg-info/[0.035] ring-1 ring-border/60 data-[switch-state=checked]:border-info/45 data-[switch-state=checked]:bg-info/12 data-[switch-state=unchecked]:border-border data-[switch-state=unchecked]:bg-info/[0.035] hover:data-[switch-state=checked]:border-info/60 hover:data-[switch-state=unchecked]:border-info/25'

const SETTINGS_SWITCH_THUMB =
  '[&>span]:relative [&>span]:z-10 [&>span]:h-[1.375rem] [&>span]:w-9 [&>span]:rounded-full [&>span]:border [&>span]:shadow-none [&>span]:transition-transform data-[switch-state=checked]:[&>span]:border-info data-[switch-state=checked]:[&>span]:bg-info data-[switch-state=unchecked]:[&>span]:border-border/70 data-[switch-state=unchecked]:[&>span]:bg-background/80'
  ...
  return (
    ...
    >
      <span
        ...
        data-switch-state={checked ? 'checked' : 'unchecked'}
        className={cn(
          'pointer-events-none block h-5 w-5 rounded-full bg-background ring-0 transition-all duration-150',
          checked

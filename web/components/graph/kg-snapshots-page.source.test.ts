...
describe('KG snapshots page layout', () => {
  ...
  it('keeps dataset-bound snapshot version selection and sticky compare actions', () => {
    ...
    expect(src).toContain('const SNAPSHOT_PRIMARY_COMPARE_CLASS =')
    expect(src).toContain('const SNAPSHOT_SECONDARY_ACTION_CLASS =')
    expect(src).toContain('快照操作')
    expect(src).toContain('bounded diff')
    expect(src).not.toContain('grid grid-cols-2 gap-2')
    expect(src).not.toContain('mt-2 h-10 w-full gap-1.5 rounded-xl')
  })
  ...
  it('keeps the snapshot toolbar in one compact row without wrapping filter controls', () => {
    ...
    expect(src).toContain("relative w-[156px] shrink-0 xl:w-[176px] 2xl:w-[196px]")
    expect(src).toContain("flex min-w-0 flex-1 items-center gap-1.5 overflow-x-auto no-scrollbar")
    expect(src).toContain("inline-flex shrink-0 items-center gap-1.5")
    expect(src).toContain("flex shrink-0 items-center gap-1.5")
    expect(src).toContain('rounded-full border-info/22 bg-info/[0.09]')
    expect(src).toContain("compactLabel: '图谱'")
    expect(src).toContain("2xl:hidden")

...
describe('chunk preview top bar messages source', () => {
  ...
  it('presents the current file identity as dense labeled metadata instead of loose text', () => {
    ...
    expect(src).toContain('currentFileIndex + 1')
    expect(src).toContain('String(visibleFileType).toUpperCase()')
    expect(src).toContain("t('topBar.fileMeta.size')")
    expect(src).toContain("t('topBar.fileMeta.parser')")
    expect(src).not.toContain("t('topBar.fileMeta.index')")
    expect(src).not.toContain("t('topBar.fileMeta.type')")
  ...
  it('formats operational errors without regex parsing that can trigger ReDoS analysis', () => {
    const src = fs.readFileSync(path.resolve(__dirname, 'top-bar.tsx'), 'utf8')

    expect(src).toContain('function splitTopBarErrorMessage(cleaned: string)')
    expect(src).toContain('cleaned.indexOf')
    expect(src).toContain('cleaned.toLowerCase()')
    expect(src).toContain("lower.includes('timeout')")
    expect(src).not.toContain('/^([^:：]{2,24})[:：]\\s*(.+)$/')
    expect(src).not.toContain('.exec(cleaned)')
    expect(src).not.toContain('/请求超时|timeout/i')
  })
})

...
describe('original preview source', () => {
  it('persists the preferred preview mode so citation-driven PDF work stays docked across inspections', () => {
    ...
    expect(src).toContain("import { ORIGINAL_PREVIEW_MODE_STORAGE_KEY } from './pdf-dock'")
    expect(src).toContain('globalThis.window.localStorage.getItem(ORIGINAL_PREVIEW_MODE_STORAGE_KEY)')

...
export function ChunkList() {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-slate-50/40 shadow-[inset_1px_0_0_rgba(255,255,255,0.6)]">
      <div className="h-12 border-b border-slate-200/70 bg-white/80 flex items-center justify-between px-4 shrink-0 gap-3 backdrop-blur">
        <span className="text-xs font-semibold text-sky-700 flex items-center gap-2">
          <Layers className="w-3.5 h-3.5" />
          切片列表
          {previewData?.total_chunks ? (
            <span className="text-[10px] text-sky-700 bg-sky-100/70 px-2 py-0.5 rounded-full">
              {previewData.total_chunks}
            </span>
          ) : null}
        </span>
        <div className="flex items-center gap-2 flex-1 justify-end">
          <div className="relative w-48">
            ...
            <input
              ...
              placeholder="搜索切片内容..."
              className="w-full h-7 pl-7 pr-2 text-[11px] rounded-lg border border-slate-200/70 bg-white/80 focus:outline-none focus:ring-2 focus:ring-sky-200"
            />
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <div className="min-h-full rounded-2xl border border-slate-200/70 bg-white/70 p-3 shadow-sm backdrop-blur ring-1 ring-white/60 space-y-3">
          {previewData?.chunks ? (
            filteredChunks.length > 0 ? (
              ...
              <div className="h-full flex flex-col items-center justify-center text-slate-300 gap-2 py-12">
                ...
                <p className="text-xs text-slate-400">未找到匹配切片</p>
              </div>
            )
          ) : isLoading ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-2 py-12">
              ...
              <p className="text-xs text-slate-500">生成预览失败</p>
              <p className="text-[10px] text-slate-400 max-w-xs text-center">{error}</p>
              <button
                type="button"
                onClick={() => runPreview()}
                className="mt-2 text-[10px] px-3 py-1 rounded-full bg-sky-100/70 text-sky-700 hover:bg-sky-100"
              >
                重试
              </button>
            </div>

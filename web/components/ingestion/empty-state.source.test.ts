...
describe('ingestion empty state source', () => {
  it('turns the truly-empty state into a report-first empty state with a three-step guide', () => {
    ...
    expect(src).toContain('onUploadSample?: () => void')
    expect(src).toContain('onUploadIngest?: () => void')
    expect(src).toContain('处理效率线框预览')
    expect(src).toContain('预检盘点入口')
    expect(src).toContain('入库风险热力图预留区')
    expect(src).toContain('<svg viewBox="0 0 640 220"')
    expect(src).toContain('还没有生成入库预检结果')
    expect(src).toContain('这个页面用于入库前摸底')
    expect(src).toContain('Sample First')
    expect(src).toContain('采样')
    ...
    expect(src).toContain('正式入库')
    expect(src).toContain('onClick={onUploadSample}')
    expect(src).toContain('onClick={onUploadIngest}')

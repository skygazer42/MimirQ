...
describe('ragas metric selector productized metric catalog', () => {
  it('surfaces necessary regression-only deterministic metrics without exposing heavy research scope', () => {
    ...
    expect(regressionSrc).toContain('RegressionMetricPicker')
    expect(regressionSrc).toContain('REGRESSION_CORE_METRIC_KEYS')
    expect(regressionSrc).toContain('高级程序化指标')
    expect(regressionSrc).toContain("metric.scopes.includes('regression')")
    expect(regressionSrc).toContain('TooltipContent')
    expect(regressionSrc).toContain('detailLabel')
    expect(regressionSrc).toContain('cursor-help')

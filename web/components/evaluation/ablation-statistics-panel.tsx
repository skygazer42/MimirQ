...
function verdict(delta: number | null): { label: string; tone: string } {
  if (delta === null) return { label: '等待 metric diff', tone: 'text-slate-500' }
  if (Math.abs(delta) < 0.01) return { label: '无明显变化', tone: 'text-slate-500' }
  if (Math.abs(delta) < 0.03) return { label: '方向性变化', tone: delta > 0 ? 'text-emerald-600' : 'text-amber-600' }
  return { label: delta > 0 ? '显著候选' : '退化风险', tone: delta > 0 ? 'text-emerald-700' : 'text-rose-600' }
}
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-card p-4 shadow-sm">
      ...
      <div className="mt-3 overflow-hidden rounded-xl border border-slate-200">
        <div className="grid grid-cols-[minmax(120px,1fr)_96px_120px_120px_120px] bg-slate-50 px-3 py-2 text-[11px] uppercase tracking-[0.12em] text-slate-500">
          <div>Metric</div>
        ...
        {rows.length ? (
          rows.map((row) => {
            ...
            return (
              <div key={row.key} className="grid grid-cols-[minmax(120px,1fr)_96px_120px_120px_120px] border-t border-slate-100 px-3 py-2 text-xs">
                <div className="truncate font-mono text-slate-900">{row.key}</div>
                <div className={cn('text-right font-mono', delta && delta > 0 ? 'text-emerald-600' : delta && delta < 0 ? 'text-rose-600' : 'text-slate-600')}>
                ...
                <div className={cn('text-right font-medium', row.significant ? 'text-emerald-700' : meta.tone)}>
          ...
          <div className="px-3 py-8 text-center text-xs text-slate-500">生成 Diff 后显示指标差异；接入 per-case scores 后可计算真实 Bootstrap CI 与 p-value。</div>
        )}
      </div>
    </section>
  )

...
describe('pdf viewer source', () => {
  ...
  it('centers the active layout box instead of only scrolling to the PDF page', () => {
    ...
    expect(src).toContain('findNearestScrollableAncestor(container)')
    expect(src).toContain('if (!scrollElement) return')
    expect(src).not.toContain("el.scrollIntoView({ behavior, block: 'center' })")

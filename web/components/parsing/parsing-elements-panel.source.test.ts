...
describe('parsing elements panel source', () => {
  it('supports kind filtering and element selection for direct review', () => {
    ...
    expect(src).toContain('const hasImageElements = useMemo(')
    expect(src).toContain("setFilterKind((current) => (current === 'all' ? 'image' : current))")
    expect(src).toContain('const visibleElements = useMemo(')
    expect(src).toContain("from '@/components/auth-image'")

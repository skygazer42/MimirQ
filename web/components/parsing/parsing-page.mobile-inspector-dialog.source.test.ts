...
describe('ParsingPage mobile inspector dialog', () => {
  it('exposes inspector/navigation helpers via WorkbenchPanelDialog on small screens', () => {
    ...
    expect(contentSrc).toContain('element.visual_kind')
    ...
    expect(contentSrc).toContain('ParsingExtractPanel')
    expect(contentSrc).toContain('activeElements')
    expect(contentSrc).toContain('documentId')
    expect(contentSrc).toContain('onSelectElement')
    expect(contentSrc).toContain('onSelectEvidence')
  })
})

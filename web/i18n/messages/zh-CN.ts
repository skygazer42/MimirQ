const zhCNMessages = {
  ...
  ChunkPreview: {
    ...
    sidebar: {
      ...
      parentChild: {
        title: 'PARENT-CHILD OPTIONS',
        syncToPipeline: '同步到 Pipeline.chunk_strategy_params',
        write: '写入',
        writeSuccess: '已写入 Pipeline.chunk_strategy_params（parent_child）',
        currentPipeline: '当前 pipeline: {value}',
        currentPipelineEmpty: '(empty)',
        ratioLabel: 'child_ratio',
        ratioAria: 'parent_child child_ratio',
        ratioHelp: 'child_size = max(chunk_size × ratio, min_child_size)',
        minChildSizeLabel: 'min_child_size',
        minChildSizeAria: 'parent_child min_child_size',
        minChildSizeHelp: 'recommended: 200–600 for text docs',
        effectiveSummary: 'effective child_size: {childSize} chars · child_overlap: {childOverlap} chars · ratio: {ratio}%',
        description: 'parent_child 会生成 parent/child 两类 chunks，并在 metadata 中写入 parent_id 与 chunk_role，便于后端 parent-child 检索/重排。',
      },
      errors: {
      ...
      stats: {
        ...
        averageLength: '平均长度',
        p95Tokens: 'P95 TOKENS',
        p95: 'P95',
        coverage: '覆盖率',
        overlapWaste: '重叠浪费',
        gaps: 'Gaps',
        largestGap: 'largest {value}',
        minMaxTokens: '最小 / 最大 TOKENS',
        minMaxLength: '最短 / 最长',
        p10: 'P10: {value}',
        qualitySignals: '质量信号',
        qualitySummary: '{shortCount} 个短切片 · {duplicateCount} 个重复（估算）',
        overlapSummary: 'overlap {ratio}%（建议 {min}-{max}%）',
        coverageSummary: 'coverage {coverage}% · waste {waste}% · gaps {gaps}',
        histogramTokens: 'TOKENS 分布',
  ...
  KGDiagnosticsPage: {
    ...
    runConfig: {
      ...
      datasetPlaceholder: 'UUID',
      hardcaseMode: 'hardcase_mode',
      hardcaseModePlaceholder: '选择 hardcase_mode',
      maxCases: 'max_cases',
      k: 'k',
      hardcasesPerFailedCase: 'hardcases_per_failed_case',
      maxFailedCasesForHardcase: 'max_failed_cases_for_hardcase',
      llmTemperature: 'llm_temperature',
      extractSkills: 'extract_skills',
      extractRelations: 'extract_relations',
      extractModePlaceholder: 'auto/on/off',
      autoExtractKg: 'auto_extract_kg',
      persistRun: 'persist_run（用于 diff）',
    },
    qualityReport: {
    ...
    runs: {
      title: 'Runs（持久化）',
      refresh: '刷新',
      hint: '需要先勾选 persist_run 才能在这里看到 runs；compare 以 run.items 的 case_id 为 key。',
      runA: 'Run A',
    ...
    compare: {
      ...
      empty: '选择并加载 run A/B 后自动生成 diff',
      diffJson: 'diff（json）',
      diffHint: '选择两个 runs 并加载后生成 diff',
      changedCases: 'changed cases (top)',

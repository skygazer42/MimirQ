...
async function createLiveDataset(
  ...
  const response = await request.post(`${apiBaseUrl()}/api/v1/datasets/`, {
    ...
    data: {
      name,
      description: 'Playwright live dataset profile proof.',
...
test.describe('live dataset profile page', () => {
  test.skip(
    !LIVE_BACKEND_ENABLED,
    'Requires PLAYWRIGHT_LIVE_BACKEND=1 and a running backend'
  )

  test('renders a real dataset profile summary and exports the live JSON report on the deployed page host', async ({
    page,
    request,
  }) => {
    ...
    const datasetName = `playwright-profile-${uniqueSuffix}`
    const filename = `profile-${uniqueSuffix}.md`
    const markdown = [
      '# Dataset Profile Proof',
      '',
      `Token PROFILE-ANCHOR-${uniqueSuffix} belongs only to this dataset profile proof document.`,
      '',
      'Owner: Piper Summary.',
    ].join('\n')
    ...
    try {
      ...
      await page.goto(`/datasets/${encodeURIComponent(datasetId)}/profile`, {
        waitUntil: 'networkidle',
      })
      await expect(
        page.getByRole('heading', { name: `数据画像 · ${datasetName}` })
      ).toBeVisible({ timeout: LIVE_EXPECT_TIMEOUT_MS })
      await expect(page.getByText('文档总数')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('Chunk 数分布（每文档）')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('Parsing provenance / 路由')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      ...
      expect(download.suggestedFilename()).toContain('.profile.json')
      ...
      expect(Number(payload.total_documents || 0)).toBeGreaterThanOrEqual(1)
      expect(Number(payload.by_file_type?.md || 0)).toBeGreaterThanOrEqual(1)

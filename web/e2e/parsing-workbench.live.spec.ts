...
async function createLiveDataset(
  ...
  const response = await request.post(`${apiBaseUrl()}/api/v1/datasets/`, {
    ...
    data: {
      name,
      description: 'Playwright live parsing dataset.',
...
test.describe('live parsing workbench', () => {
  test.skip(
    !LIVE_BACKEND_ENABLED,
    'Requires PLAYWRIGHT_LIVE_BACKEND=1 and a running backend'
  )

  test('shows a real completed dataset document inside the deployed parsing workbench', async ({
    page,
    request,
  }) => {
    ...
    const datasetName = `playwright-parsing-${uniqueSuffix}`
    const filename = `parsing-${uniqueSuffix}.md`
    const uniqueToken = `PARSING-ANCHOR-${uniqueSuffix}`
    const markdown = [
      '# Parsing Workbench Proof',
      '',
      `Token ${uniqueToken} belongs only to this parsing live proof document.`,
      '',
      'Owner: Parker Preview.',
      '',
      '- item one',
      '- item two',
    ].join('\n')
    ...
    try {
      ...
      await expect(page.getByTestId('parsing-workbench-title')).toBeVisible({
      ...
      await expect(
        page.getByTestId('parsing-selected-file-summary').getByText(filename, {
          exact: true,
        })
      ).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      ...
      await expect(
        page.getByTestId('parsing-selected-file-summary').getByText('已解析')
      ).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(
        page.getByTestId('parsing-main-panel').getByText(uniqueToken)
      ).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('解析详情')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })

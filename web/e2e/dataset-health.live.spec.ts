...
async function createLiveDataset(
  ...
  const response = await request.post(`${apiBaseUrl()}/api/v1/datasets/`, {
    ...
    data: {
      name,
      description: 'Playwright live dataset health proof.',
...
test.describe('live dataset health page', () => {
  test.skip(
    !LIVE_BACKEND_ENABLED,
    'Requires PLAYWRIGHT_LIVE_BACKEND=1 and a running backend'
  )

  test('renders a real health summary and exports the live JSON report on the deployed page host', async ({
    page,
    request,
  }) => {
    ...
    const datasetName = `playwright-health-${uniqueSuffix}`
    const filename = `health-${uniqueSuffix}.md`
    const markdown = [
      '# Dataset Health Proof',
      '',
      `Token HEALTH-ANCHOR-${uniqueSuffix} belongs only to this dataset health proof document.`,
      '',
      'Owner: Harper Health.',
    ].join('\n')
    ...
    try {
      ...
      await page.goto(`/datasets/${encodeURIComponent(datasetId)}/health`, {
        waitUntil: 'networkidle',
      })
      await expect(
        page.getByRole('heading', { name: `健康概览 · ${datasetName}` })
      ).toBeVisible({ timeout: LIVE_EXPECT_TIMEOUT_MS })
      await expect(page.getByText('文档总数')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('入库状态分布')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('格式分布')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('建议（rule-based）')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      ...
      expect(download.suggestedFilename()).toContain('.health.json')
      ...
      const payload = JSON.parse(await fs.readFile(downloadPath!, 'utf8')) as {
        schema?: string
        dataset?: { id?: string }
        health?: { profile?: { total_documents?: number } }
      }
      expect(payload.schema).toBe('mimirq.dataset_health.v1')
      expect(String(payload.dataset?.id || '')).toBe(datasetId)
      expect(Number(payload.health?.profile?.total_documents || 0)).toBeGreaterThanOrEqual(1)

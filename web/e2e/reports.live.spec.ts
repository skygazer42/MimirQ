...
async function createLiveDataset(
  ...
  const response = await request.post(`${apiBaseUrl()}/api/v1/datasets/`, {
    ...
    data: {
      name,
      description: 'Playwright live reports dataset.',
...
test.describe('live reports center', () => {
  test.skip(
    !LIVE_BACKEND_ENABLED,
    'Requires PLAYWRIGHT_LIVE_BACKEND=1 and a running backend'
  )

  test('loads a real dataset report on the deployed reports page', async ({
    page,
    request,
  }) => {
    ...
    const datasetName = `playwright-reports-${Date.now()}`
    const filename = `reports-${Date.now()}.md`
    const markdown = [
      '# Reports Workbench',
      '',
      'This document exists to validate the live reports page on a real dataset.',
      '',
      'Paragraph one keeps the profile and governance sections non-empty.',
      '',
      'Paragraph two gives enough content for chunk counts and report summaries.',
      '',
    ].join('\n')
    ...
    try {
      ...
      await expect(
        page.getByRole('heading', { name: '数据报告与审计概览' })
      ...
      await expect(page.getByText('报告状态').first()).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      ...
      await expect(page.getByText('真实后端数据')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })

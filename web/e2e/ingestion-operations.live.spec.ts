...
async function createLiveDataset(
  ...
  const response = await request.post(`${apiBaseUrl()}/api/v1/datasets/`, {
    ...
    data: {
      name,
      description: 'Playwright live ingestion operations dataset.',
...
test.describe('live ingestion operations page', () => {
  test.skip(
    !LIVE_BACKEND_ENABLED,
    'Requires PLAYWRIGHT_LIVE_BACKEND=1 and a running backend'
  )

  test('shows a real dataset summary in operation mode on the deployed page host', async ({
    page,
    request,
  }) => {
    ...
    const datasetName = `playwright-ingestion-ops-${Date.now()}`
    const filename = `ingestion-ops-${Date.now()}.md`
    const markdown = [
      '# Ingestion Operations Probe',
      '',
      'Token OPS-ANCHOR belongs only to this ingestion operations document.',
      '',
    ].join('\n')
    ...
    try {
      ...
      await page.goto(
        `/knowledge/ingestion?datasetId=${encodeURIComponent(datasetId)}`,
        { waitUntil: 'networkidle' }
      )
      await expect(page.getByRole('heading', { name: '入库中心' })).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('当前目标数据集')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(
        page.getByRole('combobox', { name: '*目标数据集' })
      ).toContainText(datasetName, {
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText(/1 文档/)).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(
        page.getByText('点击选择文件，或将文件拖拽到此处')
      ).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })

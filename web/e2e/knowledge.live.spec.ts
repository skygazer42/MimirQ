...
async function createLiveDataset(
  ...
  const response = await request.post(`${apiBaseUrl()}/api/v1/datasets/`, {
    ...
    data: {
      name,
      description: 'Playwright live knowledge workbench dataset.',
...
test.describe('live knowledge workbench', () => {
  test.skip(
    !LIVE_BACKEND_ENABLED,
    'Requires PLAYWRIGHT_LIVE_BACKEND=1 and a running backend'
  )

  test('loads a real dataset document list and runs index audit in the retrieval panel', async ({
    page,
    request,
  }) => {
    ...
    const datasetName = `playwright-knowledge-${Date.now()}`
    const filename = `knowledge-${Date.now()}.md`
    const markdown = [
      '# Knowledge Workbench',
      '',
      'This document exists to validate the live knowledge page against a real dataset.',
      '',
      'Paragraph one keeps the document non-trivial for indexing and retrieval panels.',
      '',
      'Paragraph two gives enough content for chunks and index audit metrics.',
      '',
    ].join('\n')
    ...
    try {
      ...
      await page.goto(`/knowledge?dataset=${encodeURIComponent(datasetId)}`, {
        waitUntil: 'networkidle',
      })
      await expect(
        page.getByRole('heading', { name: '知识库管理' })
      ...
      await expect(page.getByText('索引审计').nth(1)).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })

      const runButton = page.getByRole('button', { name: '运行' }).last()
      await expect(runButton).toBeVisible({ timeout: LIVE_EXPECT_TIMEOUT_MS })
      await runButton.click()

      await expect(page.getByText('正常运行')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('向量总数').first()).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('文档总数').first()).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('分片总数').first()).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })

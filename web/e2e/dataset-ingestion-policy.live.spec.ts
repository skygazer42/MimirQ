...
async function createLiveDataset(
  ...
  const response = await request.post(`${apiBaseUrl()}/api/v1/datasets/`, {
    ...
    data: {
      name,
      description: 'Playwright live ingestion policy proof.',
...
test.describe('live dataset ingestion policy page', () => {
  test.skip(
    !LIVE_BACKEND_ENABLED,
    'Requires PLAYWRIGHT_LIVE_BACKEND=1 and a running backend'
  )

  test('exports the live ingestion policy and generates a real sample preview on the deployed page host', async ({
    page,
    request,
  }) => {
    ...
    const datasetName = `playwright-ingestion-policy-${uniqueSuffix}`
    const sampleFilename = `ingestion-policy-${uniqueSuffix}.md`
    const sampleMarkdown = [
      '# Ingestion Policy Preview',
      '',
      `Token INGESTION-POLICY-${uniqueSuffix} belongs only to this sample file.`,
      '',
      'Owner: Ingrid Policy.',
    ].join('\n')
    ...
    try {
      ...
      await page.goto(`/datasets/${encodeURIComponent(datasetId)}/ingestion`, {
        waitUntil: 'networkidle',
      })
      await expect(
        page.getByRole('heading', { name: '入库策略（解析前预处理）' })
      ...
      await expect(page.getByText('入库预览（样例文件）')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })

      const downloadPromise = page.waitForEvent('download')
      await page.getByRole('button', { name: '导出脚本' }).click()
      const download = await downloadPromise
      expect(await download.failure()).toBeNull()
      expect(download.suggestedFilename()).toContain('.ingestion-policy.json')

      const downloadPath = await download.path()
      expect(downloadPath).toBeTruthy()
      const exportedPolicy = JSON.parse(await fs.readFile(downloadPath!, 'utf8')) as {
        version?: string
        rules?: Array<Record<string, unknown>>
      }
      expect(String(exportedPolicy.version || '')).toBe('1')
      expect(Array.isArray(exportedPolicy.rules)).toBe(true)

      const previewFilePath = `/tmp/${sampleFilename}`
      await fs.writeFile(previewFilePath, sampleMarkdown, 'utf8')
      await page.locator('input[type="file"]:visible').first().setInputFiles(previewFilePath)
      await page.getByRole('button', { name: '生成预览' }).click()

      await expect(page.getByText('解析后 Markdown（raw）')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(page.getByText('治理后 Markdown（clean）')).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })
      await expect(
        page
          .locator('pre')
          .filter({ hasText: `INGESTION-POLICY-${uniqueSuffix}` })
          .first()
      ).toBeVisible({
        timeout: LIVE_EXPECT_TIMEOUT_MS,
      })

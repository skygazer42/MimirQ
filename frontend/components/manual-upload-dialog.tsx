  ...
  return (
    ...
    >
      <DialogTrigger asChild>
        <Button variant="outline" className="w-full mt-3 justify-center gap-2">
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>
            先解析文档查看原始内容，再根据需要自定义切片方式（按页面、按长度或按自定义分隔符，如 Markdown 标题
            “## ”）。
          </DialogDescription>
        </DialogHeader>
                ...
                {preview.segments.map((seg) => (
                  ...
                  >
                    <div className="flex justify-between mb-1">
                      <span className="font-medium">片段 #{seg.index + 1}</span>
                      {seg.page_number && (
                        <span className="text-gray-500">页码：{seg.page_number}</span>
                      )}
                    </div>
          ...
          {preview && (
            <div className="space-y-2">
              <p className="text-sm font-medium text-gray-700">
                当前切片方案：
                <span className="font-semibold text-blue-600"> {chunkPreview.length} </span>
                个块
              </p>
              <div className="border rounded-md p-3 bg-gray-50 max-h-64 overflow-y-auto text-xs space-y-2">
                {chunkPreview.slice(0, 50).map((chunk, index) => (
                  <div
                    key={index}
                  ...
                  >
                    <div className="flex justify-between mb-1">
                      ...
                      <span className="text-gray-500">
                        长度 {chunk.content.length} 字
                        {typeof chunk.page_number === 'number' && ` · 第${chunk.page_number}页`}
                      </span>
                    </div>
                ...
                {chunkPreview.length > 50 && (
                  <p className="text-[10px] text-gray-400">
                    仅展示前 50 个块，实际共有 {chunkPreview.length} 个。
                  </p>
                )}
              </div>
            </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} disabled={!preview || isSubmitting}>

...
| 正確率 / 利用可能率 | **99.6% / 100%** |
| 平均 / P50 / P95 / P99 | **1.08s / 0.81s / 3.61s / 7.73s** |
...
<details>
<summary><b>2026-07-23 の四方向・同一問題再測定を開く</b></summary>

同一の固定 800 問を、4 つの実際の連携経路で再測定しました。

<!-- データ出典：MimirQ 直接は artifacts/changzhou_mimirq_direct_800_corefix_20260723/comparison_report.json（2026-07-23T08:18:30Z）、その他の経路は artifacts/changzhou_dify_4way_800_20260723/comparison_report.json；入力 SHA-256 5a4c67c42e8f8123774279d46af39ccc793da1b89fdea19a7359f63c8cb2fac2。 -->

| 経路 | 実行成功 | 正確率 / 利用可能率 | 回答条項カバレッジ | 回答のエビデンス裏付け | 誤エビデンス率 | 平均 / P50 / P95 |
|:---|---:|---:|---:|---:|---:|---:|
| **MimirQ 直接検索** | **800 / 800** | **99.6% / 100%** | **99.7%** | **99.8%** | 3.0% | **1.08s / 0.81s / 3.61s** |
...
| **Dify ネイティブナレッジ**² | **800 / 800** | 38.9% / 76.7% | 67.3% | 87.1% | 79.1% | 10.61s / 8.51s / 27.64s |

MimirQ の 2 つの Dify 経路の検索エビデンスカバレッジは 99.7% / 96.8% でしたが、生成回答の条項カバレッジは 81.9% / 84.0% でした。損失は知識のリコールではなくワークフローの回答生成で生じています。¹ External は並列度 4 の初回で 788 / 800、遠隔切断 11 回、上流モデル 500 が 1 回。並列度 1 で失敗問だけ再試行して回復。² ネイティブナレッジは MimirQ を経由せず、同一入力・同一アプリの 2026-07-21 の完全結果を再利用。今回はさらに 125 / 125 を抜き取り実行し、経路が引き続き有効であることを確認。

...
MimirQ 已用于**市级政务智能问答助手**，覆盖 7 个区域级 + 1 个市级知识库。最新四路复测使用输入 SHA-256 `5a4c67...fac2`，其中检索直连结果为：

| 最新结果（2026-07-23） | 结果 |
|:---|---:|
| 成功执行 | **800 / 800**，0 超时 |
| 准确 / 部分准确 / 证据不足 | **796 / 4 / 0** |
| 准确率 / 可用率 | **99.5% / 100%** |
| 平均 / P50 / P95 / P99 | **1.32s / 1.04s / 3.83s / 7.81s** |
...
<details>
<summary><b>展开 2026-07-23 四路同题复测</b></summary>

同一固定 800 题复测四种实际接入链路：

<!-- 数据来源：artifacts/changzhou_dify_4way_800_20260723/comparison_report.json；输入 SHA-256 5a4c67c42e8f8123774279d46af39ccc793da1b89fdea19a7359f63c8cb2fac2；生成时间 2026-07-23T06:57:11Z。 -->

| 链路 | 成功执行 | 准确率 / 可用率 | 答案条款覆盖 | 答案受证据支持 | 错误证据率 | 平均 / P50 / P95 |
|:---|---:|---:|---:|---:|---:|---:|
| **MimirQ 检索直连** | **800 / 800** | **99.5% / 100%** | **99.7%** | **99.8%** | 3.1% | **1.32s / 1.04s / 3.83s** |
...
| **Dify 原生知识库**² | **800 / 800** | 38.9% / 76.7% | 67.3% | 87.1% | 79.1% | 10.61s / 8.51s / 27.64s |

MimirQ 两条 Dify 链路的检索证据覆盖为 99.7% / 96.8%，但生成答案条款覆盖为 81.9% / 84.0%，主要损失在工作流生成输出而不是知识召回。¹ External 并发 4 首轮为 788 / 800，11 次远端断连、1 次上游模型 500；并发 1 仅重试失败题后恢复。² 原生知识库不经过 MimirQ，复用同输入、同 App 的 2026-07-21 完整结果，本轮另抽跑 125 / 125 验证链路仍可用。

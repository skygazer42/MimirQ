...
MimirQ 支持按 `pipeline_hash` 管理同一文档的“处理版本”（切块/清洗/解析配置变化后形成的新版本）。这类能力在 Dify / RAGFlow / FastGPT 等知识库产品中通常用于：

- 运维回滚：快速切回旧版本的切片与检索结果（不需要重新解析/重新向量化）
- 对比调参：对比不同 pipeline 配置下的切片质量与召回效果
...
- **激活版本**：用于检索与引用（citations）的版本。切换激活版本不会触发重新处理，只是改变“读”的指向。
...
- 切片列表顶部（桌面端）：可直接选择“查看版本”，在不激活的情况下预览某个版本的切片。

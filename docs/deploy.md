...
make up
make up-web   # 可选：前端 UI（profile=web）
```

## 2) 本地开发（热更新 / 源码挂载）

仓库包含 `docker-compose.override.yml`（dev override）。推荐用 Makefile 显式启动：
...
生产栈使用 `docker-compose.prod.yml`，默认不暴露基础设施端口（更安全），并启用 `ENV=production` 触发后端配置校验：
...
```bash
make up-prod
make up-prod-web   # 可选：带前端 UI（profile=web）
```

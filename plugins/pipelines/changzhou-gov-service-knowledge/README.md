...
仓库按原始知识包的六类目录保存最小演示数据：

```text
resources/
├── 01政务服务事项知识/sample.json
├── 02高效办成一件事/sample.json
├── 03常州市常见问题/sample.json
├── 04专题常见问答/sample.json
├── 05业务部门常见问题/sample.json
└── 06各区常见问题/sample.json
```

每类只保留一到三条短选编，用于说明对应 handler 的输入结构。插件根目录的 `sample.json` 是以上六份数据的聚合版本，仅供本地 runner 和 CI 一次性执行。样例选自公开政务知识包，并省略电话、详细地址和长材料清单；路径使用虚拟根目录，不包含本机或生产环境信息。样例不构成最新办事依据。

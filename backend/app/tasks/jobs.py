...
async def process_document_job(ctx, tenant_id: str, document_id: str, requested_by: str) -> dict:  # noqa: ANN001
    ...
    try:
        ...
        redis = None
    lock_key = f"lock:doc:{tenant_id}:{document_id}"
    ...
    if redis is not None:
        ...
        if not acquired:
            ...
            return {"ok": True, "skipped": "locked", "tenant_id": tenant_id, "document_id": document_id}
    ...
    try:
        ...
        if redis is not None:
            try:
                ...
                if cur is None or (isinstance(cur, (bytes, bytearray)) and cur.decode("utf-8", "ignore") == lock_val) or cur == lock_val:

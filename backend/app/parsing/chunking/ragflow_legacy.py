...
These chunkers were historically placed under `app.rag.chunkers_legacy`. To keep the
backend pipeline coherent (parse -> governance -> chunk), this wrapper provides a
single canonical entry point under `app.parsing.chunking`.
"""
...
def chunk_file(file_path: Path, *, strategy: ChunkStrategy, callback: Callable | None = None):
    ...
    if strat == "ragflow_naive":
        from app.rag.chunkers_legacy.chunkers.naive import chunk as rf_chunk
    elif strat == "ragflow_book":
        from app.rag.chunkers_legacy.chunkers.book import chunk as rf_chunk
    elif strat == "ragflow_laws":
        from app.rag.chunkers_legacy.chunkers.laws import chunk as rf_chunk
    elif strat == "ragflow_email":
        from app.rag.chunkers_legacy.chunkers.email import chunk as rf_chunk
    else:

...
| P0 | MagicPDF serviceization remote validation | Code now has a `mimirq-magicpdf` service profile and API service-mode adapter; next remote run must build/start it, set `MAGIC_PDF_API_URL=http://mimirq-magicpdf:2095/convert`, verify `settings/status` reports `configured (service)`, and parse real PDFs with `resolved_backend=magicpdf` before treating MagicPDF as production-ready. |
...
- `magicpdf` is not actually runnable in the current API container because the `magic-pdf` CLI is absent from `PATH` / runtime. Until that binary is installed into the same runtime as the API process, direct MagicPDF verification will fail immediately regardless of document size.

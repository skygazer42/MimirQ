...
- OLMOCR, Docling, and MagicPDF are functional but slow on the small 2-page smoke PDF. On the 144-page PDF, MagicPDF/Marker/PaddleOCR-VL/OLMOCR did not complete as their requested backend through MimirQ and fell back to `basic`; do not count HTTP 200 alone as advanced parser success. `scripts/remote_pdf_parser_performance.py` now treats requested/resolved backend mismatch as a failure class.

...
| #3 `connectors.py` 拆分 | 部分落地 | 根路径 connector catalog endpoint 已拆到 `app/api/v1/connectors_catalog.py`，`connectors.router` 通过 `router.routes.extend(connectors_catalog.router.routes)` 保持原 `GET /connectors` 路径不变；`tests/test_connectors_router_split_source.py` 锁定源代码拆分和运行时路由暴露；`web/scripts/api-contract-lib.mjs` 已支持该根路径拆分扫描。剩余 validate/runs/configs/scheduled 等主路由仍需继续 service 化和拆分。 |

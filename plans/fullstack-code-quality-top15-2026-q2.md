...
| 编号 | 状态 | 当前证据 |
|---|---|---|
| #1 CI PR gate | 已落地 | `ci.yml` / `security.yml` / `api-docs.yml` 已补 `pull_request` + `push main` 触发；`make verify` 覆盖 api contract/coverage。 |
...
| #5 `.env.example` 合并 | 已落地 | 根 `.env.example` 已合并为完整环境变量模板；`tests/test_env_example_split.py` 锁定核心键存在且不再引用旧拆分目录。 |
...
| CI | 10 个 workflow（ci/security/lint-fast/perf-nightly/rag-quality-gate/api-docs/handbook-matrix/parsing-proof-{nightly,sample}/docs-site） |
...
- **位置**：`.github/workflows/{ci,security,api-docs}.yml`
- **当前状态**：`pull_request` + `push: main` 已补齐，主线 PR 不再依赖人工 `workflow_dispatch`
- **证据**：`ci.yml` / `security.yml` / `api-docs.yml` 均已包含 `pull_request`；`ci.yml` 还覆盖 backend/frontend tests、Playwright smoke 与 `make verify`
...
#### 5. `.env.example` 合并已落地（原 High）
- **位置**：`.env.example`
- **当前状态**：根 `.env.example` 是完整环境变量模板,配置目录只保留项目级非 env 清单。
- **证据**：`tests/test_env_example_split.py` 锁定核心键存在且不再引用旧拆分目录。
- **后续建议**：继续约束注释质量和默认关闭策略,避免环境变量散落到多个目录。

...
#### C11(原 #17)— `.env.example` 已合并为单一完整模板
- **进展**:根 `.env.example` 已承接本地启动与高级能力配置,不再依赖拆分的 env 模板目录。
- **剩余**:继续约束模板注释质量和默认关闭策略,避免新增配置散落到多个目录。
- **修复**:新环境变量直接维护在根 `.env.example`;项目级非 env 配置保留在 `config/`。

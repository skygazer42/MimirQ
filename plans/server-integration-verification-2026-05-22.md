...
- 2026-05-24 03:44: Added `web/e2e/graph-diagnostics.live.spec.ts` plus a dedicated remote-graph-diagnostics Playwright config and verified one live `/graph/diagnostics` workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/graph-diagnostics-live-20260524-remote-web.json`) created a real dataset and completed Markdown document, ran heuristic KG extraction, listed the live document chunks, created one regression case pointing at the real `document_id + chunk_id`, opened `/graph/diagnostics`, selected the same dataset, ran `开始评测`, expanded the page's raw-result panel to confirm the live question and `baseline_hit_rate` were present, then verified the resulting persisted run appeared back in the page's run-history region and that the `历史运行` compare view could be opened. Cleanup deletes the regression case, purges the dataset, and removes the disposable documents.

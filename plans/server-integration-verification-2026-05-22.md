...
- `scripts/production_readiness_chain.py` is now proven portable on the remote host itself after `main@d358e311`, even when `python-docx` and `openpyxl` are absent. The remaining unproven portability surface is the minimal API container path where `requests` may also be missing; `scripts/remote_api_chain_smoke.py` remains the lighter-weight fallback when only a dependency-light smoke is needed.

...
- A third real-PDF summary probe now exists on the original 144-page LLM-survey
  corpus itself:
  `artifacts/real-pdf-chain/llm-survey-summary-20260523-manual/report.json`
  reused the cached `large-paper.pdf`, re-ran heuristic KG extraction for the
  same parsed document, and then exercised three generic summary prompts:
  `What is this paper about?`, `Summarize this paper in one sentence.`, and
  `What are the main themes discussed in this paper?`. That manual probe
  reported `120 events / 1148 entities / 1887 links`, and all three generic
  prompts returned non-zero `kg_search` clue counts (`90 / 90 / 90`) with
  `10` events each.
...
- The `table_store` deployment gap is now narrowed to a redeploy-only problem. `main@ab4afe05` forces `TABLE_STORE_DIR=/data/uploads/table_store` for API/worker compose variants, and a local split-container validation using the existing `docker-mimirq-api:latest` + `docker-mimirq-worker:latest` images plus the shared upload volume passed `scripts/remote_table_store_probe.py` end to end (`artifacts/table-store-probe/local-docker-20260524/report.json`). That means the current remaining gap is not “unknown root cause”; it is specifically “the remote `192.0.2.253` stack has not been redeployed with this compose fix because SSH still stops at the `deploy-user` password prompt”.
...
- 2026-05-24 15:20: Validated the `TABLE_STORE_DIR=/data/uploads/table_store` compose override locally in a real split-container setup by launching `docker-mimirq-api:latest` and `docker-mimirq-worker:latest` on a shared upload volume and then running `scripts/remote_table_store_probe.py` against `http://127.0.0.1:18080`. The resulting artifact (`artifacts/table-store-probe/local-docker-20260524/report.json`) passed list/detail/preview/query end to end on a disposable CSV dataset, confirming the compose fix is sufficient in a deployment-style environment. The unresolved part is still the remote redeploy on `192.0.2.253`, not the correctness of the fix itself.
...
- 2026-05-24 12:02: Added `scripts/remote_precheck_batch_probe.py` and verified the `upload-batch + precheck_only` path on the live API. The resulting artifact (`artifacts/precheck-batch/remote-20260524/report.json`) created a disposable dataset, uploaded one Markdown file through `/api/v1/documents/upload-batch` with `precheck_only=true`, captured `precheck_scan_run_id=1c1a8bc4-ee49-4f88-84e5-2c2ca1feb1d1`, waited for the corresponding precheck scan run to reach `completed`, and then validated the resulting summary/samples payloads (`total_files=1`, `by_file_type.md=1`, one representative sample with `text_characters=66` and `findings=['short_text']`) before deleting the dataset.
...
- 2026-05-23 23:58: Reused the same multi-question probe shape on the original 144-page LLM-survey PDF by pointing `remote_real_pdf_chain.py` at the cached `large-paper.pdf` copy and then finishing the run manually after parse completion. The resulting artifact (`artifacts/real-pdf-chain/llm-survey-summary-20260523-manual/report.json`) showed that, once heuristic KG extraction was refreshed on that corpus, all three generic summary prompts (`What is this paper about?`, `Summarize this paper in one sentence.`, `What are the main themes discussed in this paper?`) returned non-zero `kg_search` clues (`90 / 90 / 90`) with `10` events each on top of `120 events / 1148 entities / 1887 links`.
- 2026-05-24 01:57: Routed `/knowledge/feedback` through a no-SSR client shell (`page.tsx` -> `page-client.tsx`) and kept its trend/timestamp UI deterministic with UTC day keys plus post-mount relative chips. After rebuilding the remote `web` container from the clean `main@b96bd3e` clone, the dedicated feedback-workbench Playwright lane passed on the deployed page host (`artifacts/ui-clickthrough/feedback-workbench-live-20260524-remote-web.json`, `1/1` in `26.001s`). The live run created a real dataset, generated a real assistant message via extractive chat, submitted real negative feedback, opened `/knowledge/feedback`, verified the detail drawer, archived the item from the UI, and confirmed the backend feedback row flipped to `extra.archived=true`.
- 2026-05-24 02:04: Added `web/e2e/graph-snapshots.live.spec.ts` plus a dedicated remote-graph-snapshots Playwright config and verified one live `/graph/snapshots` workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/graph-snapshots-live-20260524-remote-web.json`) created a real dataset and two completed Markdown documents with different chunk strategies, verified those documents resolved to distinct `active_pipeline_hash` values, ran heuristic KG extraction for both, confirmed the backend `/api/v1/kg/snapshots/compare` result was non-empty for the two dataset-scoped hashes, then opened `/graph/snapshots?dataset_id=...`, filled `手动填写快照 A/B 哈希`, ran `后端对比`, and verified the page left its empty state and rendered live diff signals including `节点变化`, `属性变化`, `新增关系`, and `删除关系`.
- 2026-05-24 03:44: Added `web/e2e/graph-diagnostics.live.spec.ts` plus a dedicated remote-graph-diagnostics Playwright config and verified the core `/graph/diagnostics` run path on the deployed `web` + `api` stack. That first proof created a real dataset and completed Markdown document, ran heuristic KG extraction, listed the live document chunks, created one regression case pointing at the real `document_id + chunk_id`, opened `/graph/diagnostics`, selected the same dataset, ran `开始评测`, expanded the page's raw-result panel to confirm the live question and `baseline_hit_rate`, and verified the persisted run appeared back in the run-history region.
- 2026-05-24 04:00: Fixed a real backend quality-report crash for the same `/graph/diagnostics` surface. `kg_denoiser.py` had been comparing a PostgreSQL `JSON` column directly to `{}`, which first produced an invalid `json = json` operator and then a `can't adapt type 'dict'` bind failure after a partial fix. `main@5bdb2774` now routes that predicate through `_missing_relation_references_expr`, compiling to `CAST(... AS JSONB) = CAST('{}' AS JSONB)`, and the new regression test `tests/test_kg_quality_report_sql.py` locks that SQL shape. After hot-applying the same code to the current remote `api` container and restarting it, the minimal live `GET /api/v1/evaluations/kg/quality/report` probe for a one-document heuristic KG dataset returned `200` with aggregate `summary / components / noise / truncation / scope` payloads instead of `500`.
- 2026-05-24 05:09: Formalized the same `kg quality report` fix into a repeatable probe. `scripts/remote_kg_quality_report_probe.py` now creates a disposable dataset/document, waits for completion, runs heuristic KG extraction, calls `/api/v1/evaluations/kg/quality/report`, validates the aggregate payload, and cleans everything back up. The first committed artifact (`artifacts/kg-quality-report/remote-20260524/report.json`) proves the endpoint returns `200` with `summary.documents=1`, `summary.events=1`, `summary.entities=2`, and populated `components / noise / truncation / scope` sections where `scope.documents_allowed=1`, turning the earlier one-off hotfix verification into a reusable server-side regression proof.
- 2026-05-24 04:00: Re-ran `web/e2e/graph-diagnostics.live.spec.ts` after the backend fix and expanded it to `2/2`. The updated artifact (`artifacts/ui-clickthrough/graph-diagnostics-live-20260524-remote-web.json`) keeps the original diagnostics-run/history proof and adds a second deployed-page-host proof for the `抽取数据` tab: after creating a real dataset/document and running heuristic KG extraction, the page can `拉取质量报告`, render the `聚合指标` card group, and expose a raw JSON payload containing `summary`, `events`, and `scope`. Cleanup deletes the regression case and purges/deletes the disposable dataset(s).
- 2026-05-24 02:13: Added `web/e2e/diagnostics.live.spec.ts` plus a dedicated remote-diagnostics Playwright config and verified one live `/diagnostics` workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/diagnostics-live-20260524-remote-web.json`) created a real dataset and completed Markdown document, preflighted both `/api/v1/rag/prompt-preview` and `/api/v1/observability/embedding-drift/snapshot` for that dataset, opened `/diagnostics`, selected the same dataset, ran `运行 RAG 预览`, expanded the page's merged raw-response panel to confirm the live `dataset_id`, `rag_preview`, and unique diagnostics anchor appeared, then ran `漂移检查` and verified the same raw-response panel changed from `embedding_drift=null` to a populated object.
- 2026-05-24 02:18: Added `web/e2e/governance-profiles.live.spec.ts` plus a dedicated remote-governance-profiles Playwright config and verified one live `/data-governance/profiles` CRUD workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/governance-profiles-live-20260524-remote-web.json`) used the real drawer UI to create a disposable custom governance profile (name + key + description), confirmed the backend profile list returned the new custom row, then filtered the page list to that unique key and removed it through the row-level `更多操作 -> 删除` flow. The same run verified the backend list no longer contained that key afterward, while a `finally` cleanup kept an API delete fallback to avoid leaving stray profiles behind on failed UI runs.
- 2026-05-24 04:49: Re-ran the dedicated remote-governance-profiles lane and extended it to a second real workflow. The updated artifact (`artifacts/ui-clickthrough/governance-profiles-live-20260524-remote-web.json`) now passes `2/2` in `9.946s`: the original custom-profile create/delete proof still passes, and a second test creates a disposable custom profile through the live API, opens that same profile in the real drawer UI, switches to `沙盒测试`, pastes a TOC-heavy Markdown sample, clicks `运行`, and verifies the deployed page renders both the cleaned Markdown result and a unified diff that removes `1. 目录 ...... 12`. Cleanup deletes the disposable profile through the live API.
- 2026-05-24 02:25: Added `web/e2e/industry-rules.live.spec.ts` plus a dedicated remote-industry-rules Playwright config and verified one live `/governance/industry-rules` workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/industry-rules-live-20260524-remote-web.json`) first preflighted the live `industrial_control` ruleset through `/api/v1/industry-rules/rulesets` and `/api/v1/industry-rules/preview-rewrite`, then opened `/governance/industry-rules`, selected the same ruleset, verified the rewrite preview expanded `RS-485 授权报错怎么办` into a query containing `RS-485 通讯线`, and finally exported the current ruleset through the real UI download flow, confirming the browser received `industrial_control.json` and that the downloaded JSON payload still named the live ruleset.
- 2026-05-24 03:47: Re-ran the dedicated remote-industry-rules lane and extended it to a second real workflow. The updated artifact (`artifacts/ui-clickthrough/industry-rules-live-20260524-remote-web.json`) now passes `2/2` in `21.969s`: the original ruleset load / rewrite preview / export proof still passes, and a second test temporarily enables metrics logging, creates a disposable dataset plus completed Markdown document, sends a real chat query containing `PLC9`, waits until the dataset-scoped `rule-suggestions` API surfaces `PLC9` as a glossary candidate, then opens `/governance/industry-rules`, selects the same ruleset and dataset, clicks the candidate-panel `刷新`, and verifies the deployed page renders both the `PLC9` candidate and the selected dataset badge. Cleanup deletes the disposable conversation, purges and deletes the dataset, and restores the original observability settings.
- 2026-05-24 02:36: Added `web/e2e/observability.live.spec.ts` plus a dedicated remote-observability Playwright config and verified one live `/observability` workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/observability-live-20260524-remote-web.json`) opened the page's real `观测运维操作` panel and drove four live backend actions through it: `依赖健康`, `任务队列`, `SLO`, and the advanced `Trace 上报`. After each click, the proof expanded the shared raw-response panel and verified the returned payload markers for the corresponding backend endpoint (`mimirq.observability.deps.v1`, `mimirq.task_queue_observability.v1`, `mimirq.slo_snapshot.v1`, and `{ reported: true, event: manual_observability_probe }`). This gives the first deployed-frontend proof for observability operations without depending on metrics-log enablement.
- 2026-05-24 03:29: Rebuilt the remote `mimirq-api` container from clean clone `/tmp/MimirQ-main-obsv` at `main@0f4aedb5`, then re-ran the dedicated remote-observability Playwright lane. The updated artifact (`artifacts/ui-clickthrough/observability-live-20260524-remote-web.json`) now passes `2/2` in `21.816s`: the original ops-panel proof still passes, and a second browser-side proof temporarily enables metrics logging through `/settings`, seeds one grounded extractive chat plus one keyword zero-hit extractive chat, then verifies the deployed `/observability` page itself reflects the live counters on both tabs (`RAG Trace` in summary, `Requests` + `Unique query_hash` + shortened zero-hit `query_hash` in Query Analytics). Cleanup deletes both disposable conversations, purges and deletes the dataset, and restores the original observability settings.
- 2026-05-24 03:28: Fixed a real observability blind spot in `main@0f4aedb5`: the explicit extractive chat path persisted answers and citations but did not emit a `rag_trace`, so `/observability/rag-metrics/summary` and `/observability/rag-metrics/query-analytics` could not see extractive or degraded chat traffic even when metrics logging was enabled. The fix landed in `app/services/chat_persistence.py`, where finalize now emits a minimal, PII-safe `rag_trace` when `generation_fallback_kind=extractive_retrieval_summary`. The new proof script `scripts/remote_observability_metrics_probe.py` then verified the deployed server behavior after rebuilding `docker-mimirq-api-1` from clean clone `/tmp/MimirQ-main-obsv`: with `ENABLE_METRICS_LOG` temporarily enabled, one grounded extractive chat plus one keyword zero-hit extractive chat moved summary `rag_trace_count` from `1` to `3`, moved query-analytics `rag_trace_count` from `1` to `3`, and moved `zero_hit_count` from `0` to `1` in `artifacts/observability-metrics/remote-20260524/report.json`. The probe also restores the original observability settings and cleans up both conversations plus the disposable dataset.
- 2026-05-24 02:49: Added `web/e2e/evidence-workbench.live.spec.ts` plus a dedicated remote-evidence Playwright config and verified one live `/knowledge/evidence` workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/evidence-workbench-live-20260524-remote-web.json`) created a real dataset and completed Markdown document, scoped the evidence query to that dataset, proved `has_evidence=true` on a real retrieve-only lookup, verified the citations panel rendered the retrieved excerpt, then ran `数据集 Drift` in the same page and confirmed the operation result payload included the live dataset id.
...
- 2026-05-24 00:46: Broadened `scripts/remote_kb_format_matrix.py` with `yaml` and `xml` fixtures and re-ran it as `artifacts/kb-format-matrix/remote-broader-20260523/report.json`. The new run covered `8` file families in one dataset (`md`, `html`, `csv`, `json`, `yaml`, `xml`, `docx`, `xlsx`), and all `8` returned `completed` status, non-empty parsed content, at least one chunk, and successful dataset-scoped retrieve + extractive chat results before cleanup purged `8` documents and deleted the dataset.
- 2026-05-24 07:52: Extended `scripts/remote_kb_format_matrix.py` with `txt`, `ini`, and `sql` fixtures plus explicit `parser_backend='basic'` overrides, but kept the default broader matrix stable by adding `--case-name` selection instead of silently changing the default corpus. The targeted live rerun (`artifacts/kb-format-matrix/remote-text-families-20260524/report.json`) selected just those three new cases and proved all `3` completed with resolved text parsing, exactly `1` chunk each, non-empty parsed content (`69 / 57 / 72` chars), and successful dataset-scoped retrieve + extractive chat with self-citations only before cleanup purged the `3` documents and deleted the dataset. A separate exploratory full `11`-case mixed-corpus rerun surfaced an `xlsx` retrieval drift, so only the targeted text-family extension was promoted into the passing ledger.
- 2026-05-24 10:03: Extended `scripts/remote_kb_format_matrix.py` again with five rarer text-like families: `toml`, `properties`, `env`, `rst`, and `log`. As with the earlier `txt/ini/sql` pass, the new fixtures are available behind explicit `--case-name` selectors with `parser_backend='basic'`, so the default broader matrix stays stable while targeted proofs can expand file-type coverage safely. The targeted live rerun (`artifacts/kb-format-matrix/remote-rare-text-20260524/report.json`) selected just those five new cases and proved all `5` completed with exactly `1` chunk each, non-empty parsed content (`58 / 50 / 47 / 77 / 67` chars), and successful dataset-scoped retrieve + extractive chat with self-citations only before cleanup purged the `5` documents and deleted the dataset.
- 2026-05-24 10:37: Extended `scripts/remote_kb_format_matrix.py` one more step with five rarer code/config-like text families: `jsonl`, `proto`, `graphql`, `terraform`, and `patch`. These also stay behind explicit `--case-name` selectors with `parser_backend='basic'`, preserving the stability of the default broader matrix while giving us targeted proof lanes for less common but still product-relevant file types. The targeted live rerun (`artifacts/kb-format-matrix/remote-rare-code-20260524/report.json`) selected just those five new cases and proved all `5` completed with exactly `1` chunk each, non-empty parsed content (`62 / 74 / 64 / 55 / 51` chars), and successful dataset-scoped retrieve + extractive chat with self-citations only before cleanup purged the `5` documents and deleted the dataset.
- 2026-05-24 11:03: Extended `scripts/remote_kb_format_matrix.py` with one office-like binary family, `pptx`, still behind explicit `--case-name` selection so the broader default matrix remains stable. The targeted live rerun (`artifacts/kb-format-matrix/remote-pptx-20260524/report.json`) generated a one-slide deck locally with a unique token/owner pair, uploaded it to the remote API, and proved the document completed with `1` chunk, `103` parsed chars, dataset-scoped retrieve `200`, extractive chat `200`, and self-citations only before cleanup purged the disposable document and deleted the dataset.
- 2026-05-24 11:28: Extended `scripts/remote_kb_format_matrix.py` again with five additional misc text-like families: `ndjson`, `hcl`, `adoc`, `diff`, and `atom`. These also stay behind explicit `--case-name` selection. The targeted live rerun (`artifacts/kb-format-matrix/remote-rare-misc-20260524/report.json`) proved all `5` completed with exactly `1` chunk each and non-empty parsed content (`61 / 58 / 81 / 35 / 129` chars). Dataset-scoped retrieve + extractive chat returned `200` for every case, the expected document appeared in citations for all five, and four of the five returned self-citations only. Cleanup purged the `5` disposable documents and deleted the dataset.
- 2026-05-24 11:46: Added a dedicated deployed-page-host proof for `/knowledge/similarity`. The new live spec creates a disposable dataset plus one completed Markdown document, waits until `ragviz/similarity/collections` exposes the corresponding `dataset_chunks` collection, preflights `ragviz/similarity/calculate` on that collection against itself, then opens `/knowledge/similarity`, selects the same collection on both axes, runs `计算相似度`, and verifies the page renders the real result with `平均相似度`, `最高相似度`, `高相似占比`, plus the expected collection label in the heatmap header. The resulting artifact is `artifacts/ui-clickthrough/similarity-workbench-live-20260524-remote-web.json` (`1/1`, `15.950s`).
- 2026-05-24 11:59: Added a dedicated deployed-page-host proof for `/knowledge/ingestion?mode=execution-monitor&datasetId=...`. The new live spec creates a disposable dataset plus one completed Markdown document through the live API, opens the execution-monitor mode on the deployed page host, and verifies the page renders `执行监控`, `解析成功率`, `任务列表`, the uploaded filename, and an `已完成` status cell inside the real task table. The resulting artifact is `artifacts/ui-clickthrough/ingestion-monitor-live-20260524-remote-web.json` (`1/1`, `17.465s`).
- 2026-05-24 12:15: Added a second deployed-page-host proof for `/knowledge/ingestion`, this time for the default operation mode. The new live spec creates a disposable dataset plus one completed Markdown document, opens `/knowledge/ingestion?datasetId=...` on the deployed page host, and verifies the page renders `入库中心`, `当前目标数据集`, the bound dataset name in the dataset combobox, a `1 文档` summary signal, and the main upload dropzone prompt. The resulting artifact is `artifacts/ui-clickthrough/ingestion-operations-live-20260524-remote-web.json` (`1/1`, `17.277s`).
- 2026-05-24 12:28: Added a dedicated deployed-page-host proof for `/evaluations?tab=regression`. The new live spec creates a disposable dataset plus one completed Markdown document, creates one real regression case tagged `golden` with one real reference source, opens the regression tab on the deployed page host, selects the dataset, verifies the question appears under `Golden 评测集`, clicks `运行 Golden`, and then confirms a new dataset-scoped regression run appears both through the live API and in the page's run timeline (`运行 {id}`). The resulting artifact is `artifacts/ui-clickthrough/regression-workbench-live-20260524-remote-web.json` (`1/1`, `17.838s`).
- 2026-05-24 12:44: Added a dedicated deployed-page-host proof for `/evaluations/ablations`. The new live spec creates a disposable dataset plus one completed Markdown document, creates one real `golden` regression case, then creates two completed retrieval-only regression runs against that dataset (`keyword` as base, `hybrid` as target). On the deployed ablations page it selects the dataset, clicks `生成对比`, switches to `原始数据`, and verifies the raw diff payload contains the real `base_run_id`, `target_run_id`, and `metric_diffs` structure. The resulting artifact is `artifacts/ui-clickthrough/retrieval-ablations-live-20260524-remote-web.json` (`1/1`, `19.923s`).
- 2026-05-24 12:52: Added a dedicated deployed-page-host proof for `/evaluations?conversation_id=...`. The new live spec creates a disposable dataset plus one completed Markdown document, sends a real extractive chat query to create a live conversation, opens the conversation-evaluation tab on the deployed page host, clicks `开始评测`, and then confirms the conversation-scoped RAGAS run count increases through the live API before verifying the page renders the new run card for that unique question with a live status badge. The resulting artifact is `artifacts/ui-clickthrough/conversation-evaluation-live-20260524-remote-web.json` (`1/1`, `17.149s`).
- 2026-05-24 12:46: Added a dedicated deployed-page-host proof for `/data-governance/common-lines`. The new live spec creates a disposable dataset plus three completed Markdown documents that share one unique header line, creates a disposable custom governance profile, preflights `/api/v1/pipeline/learn-common-lines` to confirm the candidate exists, then opens the common-lines page on the deployed page host, selects the same dataset and profile, runs `扫描`, verifies the candidate row plus `扫描 3/3 文档`, selects that row, clicks `写入配置`, and finally confirms through the live API that the governance profile now contains the expected regex rule. The resulting artifact is `artifacts/ui-clickthrough/common-lines-workbench-live-20260524-remote-web.json` (`1/1`, `52.760s`).
- 2026-05-24 12:53: Added a dedicated deployed-page-host proof for `/datasets`. The new live spec opens the datasets workbench on the deployed page host, creates one real dataset through the UI, confirms through the live API that the dataset record exists, filters the list down to that unique dataset, edits the description through the inspector edit flow, confirms the updated description through the live API, and finally deletes the dataset from the UI while verifying the record disappears from the live dataset list. The resulting artifact is `artifacts/ui-clickthrough/datasets-workbench-live-20260524-remote-web.json` (`1/1`, `16.477s`).
- 2026-05-24 13:05: Added a dedicated deployed-page-host proof for `/parsing`. The new live spec creates a disposable dataset plus one real completed Markdown document, opens the parsing workbench on the deployed page host, switches the dataset-scope selector to that dataset, waits for the backend-synced library file to appear, clicks it, and verifies the page renders the selected-file summary, an `已解析` status chip, the persisted markdown content containing the unique parsing anchor, and the library-file inspector sections (`解析详情`, `快捷操作`). The resulting artifact is `artifacts/ui-clickthrough/parsing-workbench-live-20260524-remote-web.json` (`1/1`, `20.117s`).
- 2026-05-24 13:13: Added a dedicated deployed-page-host proof for `/datasets/{id}/precheck`. The new live spec creates a disposable dataset, uploads one Markdown file through `upload-batch` with `precheck_only=true`, waits for the resulting precheck scan run to reach `completed`, opens the dataset precheck page on the deployed page host, verifies the selected dataset and summary surface render, then clicks the representative-samples `加载` button and confirms both the `预检代表性样本` table and the real filename/`short_text` bucket appear. The resulting artifact is `artifacts/ui-clickthrough/dataset-precheck-live-20260524-remote-web.json` (`1/1`, `2.966s`).
- 2026-05-24 13:18: Added a dedicated deployed-page-host proof for `/datasets/{id}/profile`. The new live spec creates a disposable dataset plus one real completed Markdown document, opens the dataset profile page on the deployed page host, verifies the live summary surfaces (`文档总数`, `Chunk 数分布（每文档）`, `Parsing provenance / 路由`), then triggers `导出 JSON` and parses the downloaded payload to confirm it contains a real summary with `total_documents>=1` and `by_file_type.md>=1`. The resulting artifact is `artifacts/ui-clickthrough/dataset-profile-live-20260524-remote-web.json` (`1/1`, `16.673s`).
- 2026-05-24 13:24: Added a dedicated deployed-page-host proof for `/datasets/{id}/health`. The new live spec creates a disposable dataset plus one real completed Markdown document, opens the dataset health page on the deployed page host, verifies the live summary surfaces (`文档总数`, `入库状态分布`, `格式分布`, `建议（rule-based）`), then triggers `导出 JSON` and parses the downloaded payload to confirm it contains the real `mimirq.dataset_health.v1` schema, the current dataset id, and a health profile with `total_documents>=1`. The resulting artifact is `artifacts/ui-clickthrough/dataset-health-live-20260524-remote-web.json` (`1/1`, `16.594s`).
- 2026-05-24 13:30: Added a dedicated deployed-page-host proof for `/datasets/{id}/workflow`. The new live spec creates a disposable dataset, opens the dataset workflow page on the deployed page host, verifies the live workflow title/badge/export-version surface, triggers `Export JSON`, parses the downloaded config payload to confirm the real dataset id/name/version/config fields, then feeds that same file back into the hidden import input and confirms the import dialog opens with parsed `Top-level keys`, including `pipeline` and `ingestion_policy`. The resulting artifact is `artifacts/ui-clickthrough/dataset-workflow-live-20260524-remote-web.json` (`1/1`, `3.606s`).
- 2026-05-24 13:46: Added a dedicated deployed-page-host proof for `/datasets/{id}/ingestion`. The new live spec creates a disposable dataset, opens the dataset ingestion-policy page on the deployed page host, verifies the live rule-list and sample-preview surfaces, triggers `导出脚本`, parses the downloaded ingestion-policy JSON (`version=1`, `rules` array), then uploads a real sample Markdown file into the page’s preview flow and confirms the page renders both `解析后 Markdown（raw）` and `治理后 Markdown（clean）`, with the unique preview token visible in the raw preview output. The resulting artifact is `artifacts/ui-clickthrough/dataset-ingestion-policy-live-20260524-remote-web.json` (`1/1`, `4.776s`).
- 2026-05-24 14:09: Added a dedicated deployed-page-host proof for `/datasets/{id}/evidence`. The new live spec creates a disposable dataset plus one real completed Markdown document, preflights a real evidence retrieval, opens the dataset evidence workbench on the deployed page host, creates one real Evidence Suite through the UI, verifies that suite exists through the live API, then creates a draft Evidence Item through the live API from the retrieved citation and uses the page’s own refresh/render path to confirm the query appears in the item list with `draft` state. The resulting artifact is `artifacts/ui-clickthrough/dataset-evidence-workbench-live-20260524-remote-web.json` (`1/1`, `19.004s`).
- 2026-05-24 14:45: Added a dedicated deployed-page-host proof for `/knowledge/{id}/health`. The new live spec creates a disposable dataset plus one real completed Markdown document, runs a real synchronous KG extraction for that document, fetches the live health payload through `/api/v1/documents/{id}/health`, then opens the document health page on the deployed page host and verifies the page renders the real filename/status plus the three core summary surfaces `解析`, `分块`, `KG`, and `检索命中（最近）`. The resulting artifact is `artifacts/ui-clickthrough/document-health-live-20260524-remote-web.json` (`1/1`, `17.089s`).
- 2026-05-24 15:00: Added a dedicated deployed-page-host proof for `/knowledge/nebula`. The new live spec creates a disposable dataset plus one real completed Markdown document, precomputes the expected global nebula chunk count from the same live `/documents` and `/documents/{id}/chunks` APIs the page consumes, opens `/knowledge/nebula` on the deployed page host, and verifies the overlay renders the live `语义星云` summary with a non-empty `真实节点` chunk count and the exact `当前共 N 个后端切片参与布局。` sentence derived from backend data. The resulting artifact is `artifacts/ui-clickthrough/knowledge-nebula-live-20260524-remote-web.json` (`1/1`, `17.925s`).
...
- 2026-05-24 06:46: Added `web/e2e/usage.live.spec.ts` plus a dedicated remote-usage Playwright config and verified one focused live `/usage` workflow against the deployed `web` + `api` stack. The artifact (`artifacts/ui-clickthrough/usage-live-20260524-remote-web.json`) opens the page-hosted tenant quota panel, checks that the top-level totals match live `/api/v1/usage/chat/tokens/summary`, `/usage/chat/cost/summary`, and `/usage/tenant/quotas` payloads, then expands the raw-response section inside `租户级配额状态` and verifies the rendered JSON includes `documents`, `storage`, `embedding_chars`, and `qps`. This turns `/usage` from a shell-only management-page proof into a panel-level proof for the tenant quota surface itself.

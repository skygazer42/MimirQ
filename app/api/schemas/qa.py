...
class DocumentQAGenerateRequest(BaseModel):
    ...
    replace_existing: bool = Field(
        default=True, description="Delete existing QA chunks (file_type=qa) in active version first"
    )
    prefer_llm: bool = Field(
        default=True, description="Prefer LLM generation when configured; otherwise fall back to extraction"
    )
    max_source_chars: int = Field(
        default=12_000, ge=500, le=200_000, description="Max chars used as LLM/extraction source"
    )

        ...
        if doc_key and doc_key in short_doc_ids:
            ...
            for row in rows:
                ...
                if getattr(row, "page_number", None) is not None:
                    stored_meta.setdefault("page", getattr(row, "page_number"))
                if getattr(row, "start_char", None) is not None:
                    stored_meta.setdefault("start_char", getattr(row, "start_char"))
                if getattr(row, "end_char", None) is not None:
                    stored_meta.setdefault("end_char", getattr(row, "end_char"))

...
def _split_text_with_regex(text: str, separator: str, keep_separator: bool) -> list[str]:
    """Split text with regex, optionally keeping separator."""
    if separator:
        if keep_separator:
...
class TextSplitter(BaseDocumentTransformer, ABC):
    ...
    def __init__(
        ...
        length_function: Callable[[list[str]], list[int]] = lambda x: [len(s) for s in x],
    ...
    def _merge_splits(self, splits: Iterable[str], separator: str, lengths: list[int]) -> list[str]:
        """Combine smaller pieces into medium size chunks."""
...
class Tokenizer:
    """Tokenizer configuration for token-based splitting."""
    chunk_overlap: int

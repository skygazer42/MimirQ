...
def _tenant_id_migrations(table: str, default_tenant: str) -> list[MigrationStatement]:
    ...
    return [
        ...
        (
            ...
            f"DEFAULT CAST(:default_tenant AS uuid);",  # noqa: S608 - table names are internal static migration targets.
            params,
        ),
        # If the column existed but was nullable, backfill and harden.
        (
            f"UPDATE {table} SET tenant_id = CAST(:default_tenant AS uuid) WHERE tenant_id IS NULL;",  # noqa: S608 - table names are internal static migration targets.
            params,
        ),
        (
            f"ALTER TABLE {table} ALTER COLUMN tenant_id SET DEFAULT CAST(:default_tenant AS uuid);",  # noqa: S608 - table names are internal static migration targets.
            params,
        ),
    ...
    try:
        ...
        ddl_statements = [
            ...
            *_tenant_id_migrations("ragas_regression_items", default_tenant),
            ...
            "CREATE INDEX IF NOT EXISTS ix_ingest_dead_letters_tenant_status "
            "ON ingest_dead_letters (tenant_id, status);",
            ...
            "CREATE INDEX IF NOT EXISTS ix_documents_tenant_created_at ON documents (tenant_id, created_at);",
            ...
            "CREATE INDEX IF NOT EXISTS ix_documents_tenant_archived_at ON documents (tenant_id, archived_at);",
            "CREATE INDEX IF NOT EXISTS ix_documents_tenant_disabled_at ON documents (tenant_id, disabled_at);",
            ...
            "CREATE INDEX IF NOT EXISTS ix_conversations_tenant_updated_at ON conversations (tenant_id, updated_at);",
            "CREATE INDEX IF NOT EXISTS ix_conversations_tenant_created_at ON conversations (tenant_id, created_at);",
            ...
            "CREATE INDEX IF NOT EXISTS ix_datasets_tenant_updated_at ON datasets (tenant_id, updated_at);",
            ...
            "FOREIGN KEY (tenant_id, dataset_id) REFERENCES datasets (tenant_id, id) ON DELETE CASCADE;",
            "CREATE INDEX IF NOT EXISTS ix_connector_runs_tenant_dataset_id ON connector_runs (tenant_id, dataset_id);",
            ...
            "CREATE INDEX IF NOT EXISTS ix_dataset_categories_tenant_name ON dataset_categories (tenant_id, name);",
            ...
            "CREATE INDEX IF NOT EXISTS ix_documents_tenant_owner_id ON documents (tenant_id, owner_id);",
            "CREATE INDEX IF NOT EXISTS ix_documents_tenant_access_mode ON documents (tenant_id, access_mode);",
            "CREATE INDEX IF NOT EXISTS ix_documents_tenant_file_type ON documents (tenant_id, file_type);",
            "ALTER TABLE documents ADD COLUMN IF NOT EXISTS dedup_key VARCHAR(255);",
            "CREATE INDEX IF NOT EXISTS ix_datasets_tenant_owner_id ON datasets (tenant_id, owner_id);",
            ...
            "         row_number() OVER ("
            "           PARTITION BY tenant_id, dataset_id, lower(metadata->>'file_sha256'), "
            ...
            "SET metadata = jsonb_set("
            "  jsonb_set(COALESCE(c.metadata, '{}'::jsonb), '{pipeline_hash}', "
            "to_jsonb(COALESCE(c.metadata->>'pipeline_hash', d.metadata->>'pipeline_hash')), true),"
            "  '{doc_pipeline_key}', to_jsonb("
            "    COALESCE("
            "      c.metadata->>'doc_pipeline_key',"
            "      concat(c.document_id::text, ':', "
            "COALESCE(c.metadata->>'pipeline_hash', d.metadata->>'pipeline_hash'))"
            "    )"
            "  ), true"
            ") "
            "FROM documents AS d "
            ...
            # =========================
            "CREATE INDEX IF NOT EXISTS ix_connector_runs_tenant_created_at ON connector_runs (tenant_id, created_at);",
            ...
            "CREATE INDEX IF NOT EXISTS ix_kg_event_entities_event ON kg_event_entities (event_id);",
            "CREATE INDEX IF NOT EXISTS ix_kg_event_entities_entity ON kg_event_entities (entity_id);",
            # Users (auth)

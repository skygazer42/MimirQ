...
def normalize_query(text: str) -> NormalizedQuery:
    ...
    # 2) Path separators (Windows -> POSIX).
    s2 = s.replace("\\\\", "/")
    if s2 != s:
        rules.append("path_separators")
        s = s2

    # 3) Numbers: remove commas between digits (e.g. 1,000 -> 1000).
    s2 = _RE_NUMERIC_COMMAS.sub("", s)
    if s2 != s:
        rules.append("numeric_commas")
        s = s2

    # 4) Versions: strip leading "v" for version-like tokens (e.g. v1.2.3 -> 1.2.3).
    s2 = _RE_VERSION_PREFIX_V.sub("", s)
    if s2 != s:
        rules.append("version_prefix_v")
        s = s2

    # 5) Units (limited): canonicalize the unit's letter case only.
    ...
    # 6) Whitespace canonicalization (collapse + strip).
    s2 = " ".join(s.split())
    if s2 != s:

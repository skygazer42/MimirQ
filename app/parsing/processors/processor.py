    ...
    async def process_document(
        ...
        try:
            ...
            if parsed.chunks is not None:
                ...
                # Opt2: cross-page table/list merge (best-effort, off by default).
                if parsed_documents and bool(getattr(settings, "CROSS_PAGE_MERGE_ENABLED", False)):
                    try:
                        t0 = time.perf_counter()
                        with metrics_span("ingest.cross_page_merge", enabled=True):
                            _docs, merge_stats = merge_cross_page_items(parsed_documents)
                        _add_stage_duration("cross_page_merge", (time.perf_counter() - t0) * 1000)
                        # Persist merge stats for dashboard/debug (best-effort).
                        try:
                            meta_patch = dict(db_document.doc_metadata or {})
                            meta_patch["cross_page_merge"] = {
                                "schema": "mimirq.cross_page_merge.v1",
                                "enabled": True,
                                **{str(k): int(v) for k, v in (merge_stats or {}).items()},
                            }
                            db_document.doc_metadata = meta_patch
                            db.commit()
                            db.refresh(db_document)
                        ...
                        logger.info("Cross-page merge failed (ignored): %s", str(exc)[:200])

                # Opt6: reading-order score from position tags (best-effort, PII-minimal).
                ...
                # Opt4: parse-then-correct via external VLM backend (best-effort; off by default).
                if parsed_documents and bool(getattr(settings, "VLM_CORRECTION_ENABLED", False)):
                    try:
                        pdf_q = None
                        m0 = db_document.doc_metadata or {}
                        if isinstance(m0, dict) and isinstance(m0.get("pdf_quality"), dict):
                            pdf_q = dict(m0.get("pdf_quality") or {})
                        pages = [(d.page_content or "") for d in parsed_documents]
                        t0 = time.perf_counter()
                        ...
                        if len(corrected_pages) == len(parsed_documents):
                            for d, txt in zip(parsed_documents, corrected_pages, strict=False):
                                d.page_content = str(txt or "")
                        try:
                            meta_patch = dict(db_document.doc_metadata or {})
                            meta_patch["vlm_correction"] = {"schema": "mimirq.vlm_correction.v1", **audit.to_dict()}
                            db_document.doc_metadata = meta_patch
                            db.commit()
                            db.refresh(db_document)
                        except Exception:
                        ...
                        logger.info("VLM correction failed (ignored): %s", str(exc)[:200])

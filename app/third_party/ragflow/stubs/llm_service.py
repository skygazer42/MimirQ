"""
RAGFlow LLMBundle adapter.

Upstream RAGFlow uses a set of internal service APIs ("LLMBundle") to access
tenant-scoped LLM services. MimirQ does not ship the full RAGFlow backend,
...
fail-soft and fall back to plaintext parsing (see `ragflow/chunkers/naive.py`).
"""
...
logger = get_logger("ragflow.llm_service")
...
class LLMBundle:
    """
    Minimal LLMBundle implementation for RAGFlow-integrated chunkers.
    ...
    def describe_with_prompt(self, image_bytes: bytes, prompt: Optional[str] = None) -> str:
        ...
        if int(getattr(resp, "status_code", 0) or 0) != 200:
            ...
            logger.warning("RAGFlow vision API error %s: %s", getattr(resp, "status_code", None), body[:500])
            raise RuntimeError(f"RAGFlow vision API error {resp.status_code}: {body[:500]}")
